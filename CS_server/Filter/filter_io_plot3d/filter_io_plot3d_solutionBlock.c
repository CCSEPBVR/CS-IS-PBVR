 ...
 * @param typeId
 */
void plot3d_solutionBlock_allocateMemory(Plot3DSolutionBlock* sBlock, DataTypeId typeId) {
 ...
 * @param sBlock Plot3DSolutionBlock* the solution grid struct to work on
 */
void plot3d_solutionBlock_deAllocateMemory(Plot3DSolutionBlock* sBlock) {
 ...
 * 
 */
int plot3d_solutionBlock_readWhole(Plot3DSolutionBlock* sBlock, GridFile* gFile) {
 ...
 * @param cGrid Plot3DCoordinateGrid* the coordinate grid instance to work on
 */
void plot3d_solutionBlock_writeToArray(Plot3DSolutionBlock* sBlock, int offset, int skip) {
    filter_log(FILTER_LOG, "##### plot3d_functionGrid_writeToArray\n");
    DataArray_copyAsFloat(&(sBlock->QValues), m_values, offset, skip);
};
 ...
 */
void plot3d_solutionBlock_dump(Plot3DSolutionBlock* sBlock, FILE* file) {
    int n;
    double d, e, mx, my, mz;
    //fprintf(file, "##  Dumping Solution ncoords=%ld  \n", sBlock->ncoords);
