...
{
    kvsClassName( pbvr::FrequencyTable );
...
public:
    ...
    const kvs::Real64 minRange() const;

    const kvs::Real64 maxRange() const;

    const size_t maxCount() const;

    const kvs::Real64 mean() const;

    const kvs::Real64 variance() const;

    const kvs::Real64 standardDeviation() const;

    const kvs::UInt64 nbins() const;
...
public:
    ...
    //
    template <typename T>
    inline void create( const kvs::ValueArray<T>& vec );
...
private:
    ...
    template <typename T>
    inline void count_bin( const kvs::ValueArray<T>& vec );
    ...
    //
    template <typename T>
    inline void binning( const  kvs::ValueArray<T>& vec );
...
template <typename T>
inline void FrequencyTable::binning( const kvs::ValueArray<T>& vec )
{
    const size_t veclen = 1;
    typename kvs::ValueArray<T>::const_iterator value = vec.begin();
    typename kvs::ValueArray<T>::const_iterator end   = vec.end();
...
template <typename T>
inline void FrequencyTable::count_bin( const kvs::ValueArray<T>& vec )
{
    this->binning<T>( vec );
}

template <typename T>
inline void FrequencyTable::create( const kvs::ValueArray<T>& vec )
{
    this->count_bin( vec );
}
