...
void CellByCellUniformSampling::generate_particles( const pbvr::StructuredVolumeObject& volume )
{
    ...
    for ( int j = 0; j < nvariables; j++ )
    {
        ...
        for ( int i = 0; i < nnodes; i++ )
        {
            ...
            // øøøøø doubleøøøøøøøøøøøøøfloatøøøøøøøøøøøøfloatøøøøøøøøøøøøøøøøøø
        }
    } 
    ...
    //øøøøøø
    int nbins = 256;
    kvs::ValueArray<float> o_min( tf_number );//TFSøøøøøøøøøøø
    kvs::ValueArray<float> o_max( tf_number );
    kvs::ValueArray<float> c_min( tf_number );
    kvs::ValueArray<float> c_max( tf_number );

    kvs::ValueArray<int> o_histogram( tf_number * nbins );//øøøøøøøøøøøøø
    kvs::ValueArray<int> c_histogram( tf_number * nbins );//øøøøøøøøøø
    ...
    //øøøøø
    kvs::ValueArray<float> O_min( tf_number );//øøøøøøøøøøø
    kvs::ValueArray<float> O_max( tf_number );
    kvs::ValueArray<float> C_min( tf_number );
    kvs::ValueArray<float> C_max( tf_number );
    ...
    if( parameter_file_opened )
    {
        for ( size_t i = 0; i < tf_number; i++ ) //øøø
        {
            O_min[ i ] =  FLT_MAX;
            O_max[ i ] = -FLT_MAX;
            C_min[ i ] =  FLT_MAX;
...
//    static TimedScope td_VectorIns("Vector Insert",max_threads);
    #pragma omp parallel
    {
        ...
        //øøøøøøøøø
        float o_scalars[tf_number][SIMDW];//øøøøøøø
        float c_scalars[tf_number][SIMDW];//øøøø

        kvs::ValueArray<int> th_o_histogram( tf_number * nbins );//øøøø
        kvs::ValueArray<int> th_c_histogram( tf_number * nbins );//ø
        ...
        //øøøøø
        kvs::ValueArray<float> th_O_min( tf_number );//øøøøøøøøøøø
...
//                timed_section_end(td_CalculateHistogram,thid);
        ...
        //øøøøøøøøøøøø
        {
            ...
            for( int J=0; J<outer_loop; J++ )
            {
                float X_l[SIMDW], Y_l[SIMDW], Z_l[SIMDW];//interpøøøøøø
                float X_g[SIMDW], Y_g[SIMDW], Z_g[SIMDW];//TFSøøøøøø
                #pragma ivdep
                for( int I=0; I<SIMDW; I++ )
                {
                    const int vertex_id = I + J * SIMDW;
...
//            th_tfs[thid]->CalculateOpacityArray( interp[thid],
            ...
            for(int cell_BLK = 0; cell_BLK < remain; cell_BLK++ )
            {
                ...
                for( int i = 0; i < nparticles_array[cell_BLK]; i+=SIMD_BLK_SIZE )
                {
                    ...
                    //øøøøøcell_index, local_coordøøøø
                    int nparticles_count = 0;
                    for( int j = 0; j < remain_BLK; j++ )
                    {
                    ...
                    for( int j = 0; j < nparticles_count; j++ )
                    {
                        //JacobiMatrixøøøøøøøøøøøøøøøøøøøøø
...
//        c_histogram_recv.fill(0x00);
    ...
    //delete tfs;  // øøøøøøøøøøøøøøøøøøøøøøø
...
void CellByCellUniformSampling::calculate_histogram( kvs::ValueArray<int>&   th_o_histogram,
                          ...
                          // øøøøoutput, øøinput
                          const int nbins, // TFSøøøøøøøøøøø
                          ...
                          const float o_scalars[][SIMDW], // ÂêàÊàêÂÄ§
