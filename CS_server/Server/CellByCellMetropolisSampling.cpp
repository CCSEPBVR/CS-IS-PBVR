 ...
 */
/*===========================================================================*/
void CellByCellMetropolisSampling::mapping( const kvs::Camera& camera, const pbvr::StructuredVolumeObject& volume )
{
...
//                    timed_section_end(td_CalculateNumPar,thid);
                ...
                for(int I=0; I<SIMDW+1; I++) 
                {
                    ...
                    //float        density[SIMDW];
                    float        density;
                    kvs::Vector3f        point_trial[SIMDW];
                    ...
                    for(int ii=0; ii < max_loop; ii++)
                    {
                        ...
                        if( p_id == SIMDW )
                        {
                            ...
                            for( int pp=0; pp<SIMDW; pp++)
                            {
                                ...
                                if ( !kvs::Math::IsZero( density_tmp ) )
                                {
                                    density = density_tmp;
                                    finish_flag =true; 
                                    break;
...
void CellByCellMetropolisSampling::calculate_histogram( kvs::ValueArray<int>&   th_o_histogram,
...
kvs::Vector3f CellByCellMetropolisSampling::RandomSamplingInCube( const kvs::Vector3f vertex, kvs::MersenneTwister* MT  )
{
