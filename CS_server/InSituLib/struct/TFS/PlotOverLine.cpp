...
void PlotOverLine::extractPlotLinePoly( const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
    this->calculate_x_axis( P0, P1 );
...
void PlotOverLine::calculate_x_axis( const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
...
void PlotOverLine::for_tetrahedral_mesh( const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
    ...
    for( int cell = 0; cell < ncells; cell++, id+=4 )
    {
        ...
        kvs::Vec4 scalar(
            ...
            //m_volume->values()[ local_id[3] ].to<kvs::Real32>() );

        this->sampling_in_tetrahedra( P0, P1, vertex, scalar );
    }
}

void PlotOverLine::for_hexahedral_mesh( const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
    const kvs::Real32* coords = m_volume->coords().pointer();
    const kvs::UInt32* connec = m_volume->connections().pointer();
    const size_t ncells = m_volume->ncells();
    ...
    for( int cell = 0; cell < ncells; cell++, id+=8 )
    {
        ...
        // Bounding Box
        kvs::Vec3 MinCoord( vertex[0] );
        kvs::Vec3 MaxCoord( vertex[0] );
        for( int i=1; i<8; i++ )
        {
        ...
        kvs::Vec3 cell_center_vertex( (vertex[0]+vertex[1]+vertex[2]+vertex[3]+
...
void PlotOverLine::for_pyramidal_mesh( const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
    ...
    // for VTK
    //const kvs::UInt32 face_id[24] = { 3, 2, 1, 0 };
    ...
    for( int cell = 0; cell < ncells; cell++, id+=5 )
    {
        ...
        // for AVS
        kvs::Vec3 cell_center_vertex( vertex[0] );

        // for VTK
        //kvs::Vec3 cell_center_vertex( vertex[4] );
        ...
        // for AVS
        kvs::Real32 cell_center_scalar = ( scalar[0] );

        //for VTK
        //kvs::Real32 cell_center_scalar = ( scalar[4] ); 
...
void PlotOverLine::for_prismic_mesh( const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
    const kvs::Real32* coords = m_volume->coords().pointer();
    const kvs::UInt32* connec = m_volume->connections().pointer();
    const size_t ncells = m_volume->ncells();
    ...
    for( int cell = 0; cell < ncells; cell++, id+=6 )
    {
        ...
        kvs::Vec3 cell_center_vertex( (vertex[0]+vertex[1]+vertex[2]+
        ...
        kvs::Real32 cell_center_scalar = ((scalar[0]+scalar[1]+scalar[2]+
...
void PlotOverLine::for_polyhedral_mesh( const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
    const kvs::Real32* coords = m_polyhedron->coords.pointer();
    const kvs::UInt32* connec = m_polyhedron->connections.pointer();
    const kvs::UInt32* conoff = m_polyhedron->connection_offsets.pointer();
    const kvs::UInt32* faces  = m_polyhedron->faces.pointer();
    const kvs::UInt32* facoff = m_polyhedron->face_offsets.pointer();
    const kvs::Real32* scalar = m_polyhedron->scalars.pointer();
    ...
    for( int cell = 0; cell < ncells; cell++ )
    {
        ...
        const kvs::Vec3 vertex0( coords[id0],coords[id0+1],coords[id0+2] );
        kvs::Vec3 MinCoord( vertex0 );
        kvs::Vec3 MaxCoord( vertex0 );
        for( int i=vid0+1; i<vid1; i++ )
        {
            const size_t id = connec[i]*3;
            const kvs::Vec3 vertex( coords[id], coords[id+1], coords[id+2] );
        ...
        // Cell center vertex
        kvs::Vec3 cell_center_vertex( vertex0 );
        for( int i=vid0+1; i<vid1; i++ )
        {
            const size_t id = connec[i]*3;
            cell_center_vertex += kvs::Vec3( coords[id], coords[id+1], coords[id+2] );
        }
        const size_t ncellvert = vid1 - vid0;
        cell_center_vertex /= (kvs::Real32)ncellvert;

        // Cell center scalar
        kvs::Real32 cell_center_scalar = 0.0f;
        for( int i=vid0; i<vid1; i++ )
        {
            const size_t id = connec[i];
            cell_center_scalar += scalar[id];
        }
        cell_center_scalar /= (kvs::Real32)ncellvert;

        // Access each face
        const size_t fid0 = cell==0 ? 0 : facoff[ cell -1 ];

        const kvs::UInt32* pface_array = &faces[ fid0 ];
        ...
        {
            // Calc. Face center
            kvs::Vec3 face_center_vertex( 0.0f, 0.0f, 0.0f );
            kvs::Real32 face_center_scalar = 0.0f;
            ...
            const kvs::UInt32* pface = pface_array;

            for( int vcnt=0; vcnt<nfacevert; vcnt++ )
            {
                const size_t face_vert_id = *pface_array;
                pface_array++;
                const kvs::Vec3 vertex( coords[face_vert_id*3  ],
                                        coords[face_vert_id*3+1],
                                        coords[face_vert_id*3+2] );
                face_center_vertex += vertex;

                const kvs::Real32 scal = scalar[face_vert_id];
                face_center_scalar += scal;
            }
            face_center_vertex /= (kvs::Real32)nfacevert;
            face_center_scalar /= (kvs::Real32)nfacevert;
            ...
            for( int i=0; i<nfacevert; i++ )
            {
                ...
                vert[2] = kvs::Vec3( coords[vid2*3], coords[vid2*3+1], coords[vid2*3+2] );
                vert[3] = kvs::Vec3( coords[vid3*3], coords[vid3*3+1], coords[vid3*3+2] );
...
void PlotOverLine::sampling_in_tetrahedra( const kvs::Vec3 P0, const kvs::Vec3 P1,
                                           const kvs::Vec3* vertices,
                                           const kvs::Vec4 scalars )
{
    // Tetrahedra vertices and scalars
    const kvs::Vec3 X0 = vertices[0];
    const kvs::Vec3 X1 = vertices[1]; 
    ...
    kvs::Vec3 MinCoord( kvs::Math::Min<float>( X0.x(), X1.x(), X2.x(), X3.x() ),
                        ...
                        kvs::Math::Min<float>( X0.z(), X1.z(), X2.z(), X3.z() ) );
    kvs::Vec3 MaxCoord( kvs::Math::Max<float>( X0.x(), X1.x(), X2.x(), X3.x() ),
                        kvs::Math::Max<float>( X0.y(), X1.y(), X2.y(), X3.y() ),
                        kvs::Math::Max<float>( X0.z(), X1.z(), X2.z(), X3.z() ) );
    ...
    const kvs::Vec4 bc_P0  =  this->barycentric_coordinates( X0, X1, X2, X3, P0 );

    // Barycentric coordinates of P1 := ( a1, b1, c1, d1 )
    const kvs::Vec4 bc_P1  =  this->barycentric_coordinates( X0, X1, X2, X3, P1 );
    ...
    const int discretized_t_lower = kvs::Math::Ceil <float>( t_range.lower() * (nsamples-1) );
    const int discretized_t_upper = kvs::Math::Floor<float>( t_range.upper() * (nsamples-1) );
...
const bool PlotOverLine::intersection_of_boundingbox(
    const kvs::Vec3 MinCoord, const kvs::Vec3 MaxCoord,
    const kvs::Vec3 P0, const kvs::Vec3 P1 )
{
    // P(t) = P0 + t (P1 - P0)
    float t_min = 0.0f;
...
{
    const kvs::Vec3 X0_3 = X0 - X3;
    const kvs::Vec3 X1_3 = X1 - X3;
    const kvs::Vec3 X2_3 = X2 - X3;
    const kvs::Mat3 M(
...
const kvs::Vec4 PlotOverLine::barycentric_coordinates(
    const kvs::Vec3 X0, const kvs::Vec3 X1, const kvs::Vec3 X2, const kvs::Vec3 X3, const kvs::Vec3 P )
{
    ...
    return kvs::Vec4( V0/V, V1/V, V2/V, V3/V );
}
...
POL::Range PlotOverLine::t_range_in_tet( const kvs::Vec4 bc_P0, const kvs::Vec4 bc_P1 )
{
