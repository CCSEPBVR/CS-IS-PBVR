...
vismodule::AnyValueArray Tiff::rawData( void ) const
{
    return( m_raw_data );
}
...
bool Tiff::isSupported( void ) const
{
    ...
    {
        visModuleMessageError("Not supported compressed TIFF image.");
        ret = false;
    }
...
const bool Tiff::read( const std::string& filename )
{
    ...
    {
        visModuleMessageError( "Cannot read header." );
        ifs.close();
        BaseClass::m_is_success = false;
        return( BaseClass::m_is_success );
    }
    ...
    if ( !m_ifd.read( ifs ) )
    {
        visModuleMessageError( "Cannot read IFD." );
        ifs.close();
        BaseClass::m_is_success = false;
        return( BaseClass::m_is_success );
    }
...
size_t Tiff::get_width( void ) const
{
    ...
    const vismodule::UInt16 TIFF_IMAGE_WIDTH = 256;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_IMAGE_WIDTH ) );

    // No default.
    if ( entry == m_ifd.entryList().end() )
    {
        visModuleMessageError("Cannot find the image width in 0th-IFD.");
        return( 0 );
    }
    ...
    case vismodule::tiff::Short:
        ret = entry->values().at<vismodule::UInt16>(0); // vismodule::tiff::Short
        break;
    case vismodule::tiff::Long:
        ret = entry->values().at<vismodule::UInt32>(0); // vismodule::tiff::Long
        break;
    default:
        break;
    }
...
size_t Tiff::get_height( void ) const
{
    ...
    const vismodule::UInt16 TIFF_IMAGE_HEIGHT = 257;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_IMAGE_HEIGHT ) );

    // No default.
    if ( entry == m_ifd.entryList().end() )
    {
        visModuleMessageError("Cannot find the image height in 0th-IFD.");
        return( 0 );
    }
...
size_t Tiff::get_bits_per_sample( void ) const
{
    ...
    const vismodule::UInt16 TIFF_BITSPERSAMPLE = 258;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_BITSPERSAMPLE ) );
...
{
    // Samples per pixel
    const vismodule::UInt16 TIFF_SAMPLESPERPIXEL = 277;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_SAMPLESPERPIXEL ) );
    ...
    return( entry->values().at<vismodule::UInt16>(0) ); // vismodule::tiff::Short
}
...
size_t Tiff::get_photometirc_interpretation( void ) const
{
     ...
     *   8: 1976 CIE L*a*b*
     */
    const vismodule::UInt16 TIFF_PHOTOMETRIC = 262;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_PHOTOMETRIC ) );

    // Unknown default value.
    if ( entry == m_ifd.entryList().end() )
    {
        visModuleMessageError("Cannot find the photometric interpretation in 0th-IFD.");
        return( 0 );
    }
...
size_t Tiff::get_rows_per_strip( void ) const
{
    ...
    const vismodule::UInt16 TIFF_ROWSPERSTRIP = 278;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_ROWSPERSTRIP ) );
...
size_t Tiff::get_compression_mode( void ) const
{
     ...
     *   34661: ISO JBIG
     */
    const vismodule::UInt16 TIFF_COMPRESSION = 259;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_COMPRESSION ) );
...
vismodule::AnyValueArray Tiff::get_strip_offsets( void ) const
{
    // Offsets to the data strips.
    const vismodule::UInt16 TIFF_STRIPOFFSETS = 273;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_STRIPOFFSETS ) );

    // No default.
    if ( entry == m_ifd.entryList().end() )
    {
        visModuleMessageError("Cannot find the strip offsets in 0th-IFD.");
    }

    return( entry->values() );
}

vismodule::AnyValueArray Tiff::get_strip_bytes( void ) const
{
    // Bytes counts for strips.
    const vismodule::UInt16 TIFF_STRIPBYTECOUNT = 279;

    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry;
    entry = std::find( m_ifd.entryList().begin(),
                       m_ifd.entryList().end(),
                       vismodule::tiff::Entry( TIFF_STRIPBYTECOUNT ) );

    // No default.
    if ( entry == m_ifd.entryList().end() )
    {
        visModuleMessageError("Cannot find the strip bytes counts in 0th-IFD.");
    }
...
Tiff::ColorMode Tiff::get_color_mode( void ) const
{
    ...
    case 1:
        ...
        {
            visModuleMessageError("Incorrect the photometric interpretation.");
...
vismodule::AnyValueArray Tiff::get_raw_data( std::ifstream& ifs ) const
{
    vismodule::AnyValueArray raw_data;
...
std::ostream& operator << ( std::ostream& os, const Tiff& tiff )
{
    ...
    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator entry = tiff.ifd().entryList().begin();
    vismodule::tiff::ImageFileDirectory::EntryList::const_iterator last = tiff.ifd().entryList().end();
    while ( entry != last )
    {
        os << *entry;
