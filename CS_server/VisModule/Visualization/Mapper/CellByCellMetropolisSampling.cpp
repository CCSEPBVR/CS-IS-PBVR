...
void CellByCellMetropolisSampling::generate_particles_struct(
    domain_parameters_struct dom,
    Type** values,
    int nvariables
)
                    ...
                    // for( int p = 0; p < nparticles_I; p += SIMDW )
                    {
                        ...
                        while (1)
                        {
                            ...
                            if( pp_id == SIMDW || (I == SIMDW && pp_id > 0) )
                            {
                                ...
                                {
                                    // パディング：末尾を最後の有効要素で埋める
                                    for ( int jj = pp_id; jj < SIMDW; ++jj )
                                    {
                                        p_x_l[jj] = p_x_l[pp_id - 1];
                                        p_y_l[jj] = p_y_l[pp_id - 1];
                                        p_z_l[jj] = p_z_l[pp_id - 1];
                                        p_x_g[jj] = p_x_g[pp_id - 1];
                                        p_y_g[jj] = p_y_g[pp_id - 1];
                                        p_z_g[jj] = p_z_g[pp_id - 1];
                                    }
                                }
...
void CellByCellMetropolisSampling::generate_particles_unstruct(  domain_parameters_unstruct dom,Type** values, int nvariables,
        float* coordinates, int ncoords,
        unsigned int* connections, int ncells,
        const  vismodule::VolumeObjectBase::CellType& cellType )
{
                ...
                for( int i = 0; i < nparticles_array[cell_BLK]; i+=SIMD_BLK_SIZE )
                {
                    ...
                    for( int j = 0; j < remain_BLK; j++ ) 
                    {
                        ...
                        while(1)
                        {
                            ...
                            const float opacity = th_tfs[thid]->CalculateOpacity(
                                interp[thid],
                                point_trial,
                                global_point_trial,
                                th_tf[thid]
                            ...
                            density_trial = Generator::CalculateDensity(
                                opacity,
                                sampling_volume_inverse,
                                max_opacity,
                                max_density
