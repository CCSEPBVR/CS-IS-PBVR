...
const vismodule::Int32 num_element_node = 4;   // tetrahedra: 4頂点/セル
...
//    const vismodule::Real64 kd = 6.0;
    ...
    // 1立方体を5四面体に分割するため、セル数は hexahedra 版の5倍。
    ...
    for ( size_t z = 0; z < dim3-1; ++z )
    {
        for ( size_t y = 0; y < dim2-1; ++y )
        {
            for ( size_t x = 0; x < dim1-1; ++x )
            {
                ...
                // tetrahedra: 1立方体を5四面体へ分割する。隣接立方体と面が整合するよう
                // (x+y+z) の偶奇で分割の向きを変える(頂点順序は Hydrogen_unstruct_tetra 準拠)。
                if ( (x+y+z)%2 == 0 )
                {
