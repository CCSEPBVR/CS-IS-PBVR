...
int Hydrogen::generate_volume( void )
{
    ...
    const vismodule::Real64 kd = 1.0 + (double)member_id;   // ensemble member: kd = 1 + member_id
...
/*===========================================================================*/
/**
 *  @brief  Switches the ensemble member and regenerates the field (kd = 1 + member).
 */
/*===========================================================================*/
void Hydrogen::set_member( int member )
{
    member_id = member;
    this->generate_volume();   // arrays already allocated; refills values with new kd
}
...
Hydrogen::Hydrogen( void )
{
    ...
    cell_length = 127.0f / 255.0f;   // 256 nodes span [0,127] (same as the scaling test)
