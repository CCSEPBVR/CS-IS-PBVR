...
int main( int argc, char** argv )
{
    ...
    for(int i=0; i < hydro.ncells * celltype; i+=celltype )
    {
        std::string writing_cells =  std::to_string(hydro.connections[i]) +   " " + std::to_string(hydro.connections[i+1]) + " " + std::to_string(hydro.connections[i+2])
                                    ...
                                    /*+ " " + std::to_string(hydro.connections[i+6]) + " " + std::to_string(hydro.connections[i+7]); */ 
        //std::string writing_cells =         std::to_string(ConnectionArray[i]) +   " " + std::to_string(ConnectionArray[i+1]) + " " + std::to_string(ConnectionArray[i+2])
        //                            + " " + std::to_string(ConnectionArray[i+3]) + " " + std::to_string(ConnectionArray[i+4]) + " " + std::to_string(ConnectionArray[i+5]) 
        //                            + " " + std::to_string(ConnectionArray[i+6]) + " " + std::to_string(ConnectionArray[i+7]); 
    ...
    for(;;)
    {
        ...
        generate_particles( time_step, dom,
                            ...
                            hydro.connections, hydro.ncells, pbvr::VolumeObjectBase::CellType::Pyramid );
