...
{
    nvariables = 2;   // 非構造サンプル(ens_Hydrogen_unstruct)に合わせる

    // Including halo region in x-y boundary
    //resolution = vismodule::Vector3i( 64, 64, 128 );
    resolution = vismodule::Vector3i( 256, 256, 256 );   // 255^3 = 16,581,375 セル

    cell_length = 127.0f / ( 256 - 1 );   // 領域 0..127 を保つ（非構造と同一）
