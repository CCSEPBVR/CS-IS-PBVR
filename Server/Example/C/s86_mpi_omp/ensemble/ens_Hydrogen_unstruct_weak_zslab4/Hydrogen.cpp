...
/**
 *  @file   Hydrogen.cpp
 *  @brief  Weak-scaling variant of the Hydrogen volume generator (z-slab x4).
 *
 *  Design (single executable, per-rank size fixed):
 *    - GX x GY nodes in x,y and GZ_CELLS cells in z are FIXED constants.
 *    - mpi_per_ens is FIXED (default 4, override with env PBVR_WEAK_MPI_PER_ENS):
 *      the strong-scaling z-slab decomposition frozen at 4 splits per ensemble.
 *    - num_ensembles = mpi_size / mpi_per_ens GROWS with the rank count, so the
 *      per-rank grid (GX,GY,chunk+1), chunk = GZ_CELLS / mpi_per_ens, stays FIXED
 *      (weak scaling): more ranks add more ensembles, not finer per-rank grids.
 *    - One ensemble's z extent is partitioned into mpi_per_ens contiguous slabs
 *      (z-axis 1D decomposition), offset by origin_z; the union of an ensemble's
 *      ranks is always GX x GY x GZ_CELLS (seamless, shared boundary nodes).
...
Hydrogen::Hydrogen( void )
{
    ...
    // Ranks per ensemble is FIXED for weak scaling (z-slab x4, default 4).
    mpi_per_ens = 4;
    const char* env_mpe = std::getenv( "PBVR_WEAK_MPI_PER_ENS" );
    if ( env_mpe != NULL )
    {
        const int e = std::atoi( env_mpe );
        if ( e >= 1 ) mpi_per_ens = e;
    }
    if ( mpi_per_ens < 1 ) mpi_per_ens = 1;

    // Number of ensembles grows with the rank count (weak scaling).
    num_ensembles = mpi_size / mpi_per_ens;
    if ( num_ensembles < 1 ) num_ensembles = 1;

    if ( mpi_rank == 0 )
    {
        std::cout << "[weak] mpi_size=" << mpi_size
                  << " mpi_per_ens=" << mpi_per_ens
                  << " num_ensembles=" << num_ensembles
                  << " (GX=" << GX_NODES << ",GY=" << GY_NODES
                  << ",GZ_CELLS=" << GZ_CELLS << ")" << std::endl;
        if ( mpi_size % mpi_per_ens != 0 )
            std::cout << "[weak][WARN] mpi_size not divisible by mpi_per_ens" << std::endl;
        if ( GZ_CELLS % mpi_per_ens != 0 )
            std::cout << "[weak][WARN] GZ_CELLS not divisible by mpi_per_ens" << std::endl;
    }
