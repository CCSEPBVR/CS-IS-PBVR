 ...
 */
/*===========================================================================*/
const std::size_t Stl::ntriangles( void ) const
{
...
const bool Stl::read_binary( FILE* ifs )
{
    ...
    const std::size_t HeaderLength = 80;
...
{
    const char* header = "Generated by VIS_MODULE";
    fprintf( ofs, "solid %s\n", header );

    const std::size_t ntriangles = m_normals.size() / 3;
    std::size_t index3 = 0;
    std::size_t index9 = 0;
    for ( std::size_t i = 0; i < ntriangles; i++, index3 += 3, index9 += 9 )
    {
...
const bool Stl::write_binary( FILE* ofs )
{
    ...
    sprintf( header, "Generated by VIS_MODULE" );
...
const bool Stl::CheckFileFormat( const std::string& filename )
{
    ...
    if ( !::IsAsciiType( ifs ) )
    {
        ...
        const std::size_t byte_size = file.byteSize();
        ...
        const std::size_t data_size = 50 * ntriangles + 80 + 4;
