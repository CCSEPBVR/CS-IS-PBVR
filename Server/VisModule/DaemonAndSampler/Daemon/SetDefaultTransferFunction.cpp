...
void setDefalutTransferFunctionToArgument( ParticleProperty& param, const VariableRange vr, const int tf_number )
{
    ...
    for ( size_t i = 0; i < tf_number; i++ )
    {
        ...
        var_o.push_back( param.m_transfunc_synthesizer->convert_token( OSynthBuf ));
        var_c.push_back( param.m_transfunc_synthesizer->convert_token( CSynthBuf ));
    }

    param.m_transfunc_synthesizer->setOpacityVariable( var_o );
    param.m_transfunc_synthesizer->setColorVariable( var_c );
    ...
    for ( size_t i = 0; i < tf_number; i++ )
    {
        ...
        param->m_named_transfunc_array[i].m_color_variable_min   = vr.min( tt.str() + "_var_c" );
        param->m_named_transfunc_array[i].m_color_variable_max   = vr.max( tt.str() + "_var_c" ); 
        param->m_named_transfunc_array[i].m_opacity_variable_min = vr.min( tt.str() + "_var_o" );
        param->m_named_transfunc_array[i].m_opacity_variable_max = vr.max( tt.str() + "_var_o" ); 
        param->m_named_transfunc_array[i].m_resolution           = TF_resolution;
        param->m_named_transfunc_array[i].m_equation_red         = ""; 
        ...
        */
        ...
        // spx
        // vismodule::ColorMap color_map( cc_table, 0.2, 1 );
        // vismodule::OpacityMap opacity_map( oo_table, 0.2, 1 );

        // mej 
        vismodule::ColorMap color_map( cc_table    , 305, 400 );
        vismodule::OpacityMap opacity_map( oo_table, 300, 400 );

        param.m_transfunc_array[i].setColorMap( color_map );
        param.m_transfunc_array[i].setOpacityMap( opacity_map );

        // spx
        // param.m_transfunc_array[i].setColorRange( 0.2, 1 );
        // param.m_transfunc_array[i].setOpacityRange( 0.2, 1 );

        // mej
        param.m_transfunc_array[i].setColorRange( 305, 400 );
        param.m_transfunc_array[i].setOpacityRange( 300, 400 );
