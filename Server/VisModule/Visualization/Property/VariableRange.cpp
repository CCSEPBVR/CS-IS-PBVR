...
size_t VariableRange::byteSize() const
{
    ...
    {
        s += vismodule::Serializer::byteSize( i->first );
        s += vismodule::Serializer::byteSize( i->second );
    }
    s += vismodule::Serializer::byteSize( m_max.size() );
...
size_t VariableRange::pack( char* buf ) const
{
    ...
    index += vismodule::Serializer::write( buf + index, m_max.size() );
    for ( range_map_t::const_iterator i = m_max.begin(); i != m_max.end(); i++ )
    {
        index += vismodule::Serializer::write( buf + index, i->first );
        index += vismodule::Serializer::write( buf + index, i->second );
