...
void CellByCellUniformSampling::generate_particles_struct(
    domain_parameters_struct dom,
    Type** values,
    int nvariables
)
...
void CellByCellUniformSampling::generate_particles_unstruct(
    domain_parameters_unstruct dom,
    Type** values,
    int nvariables,
    float* coordinates,
...
#pragma omp critical
    ...
    for ( std::size_t i = 0; i < (tf_number * DEFAULT_NBINS); i++ )
    {
        ...
        {
            std::cout << "m_c_histogram[" << (i / DEFAULT_NBINS) << "]:";
        }

        std::cout << m_c_histogram[i];

        if ( (i % DEFAULT_NBINS) != (DEFAULT_NBINS - 1) )
        {
            std::cout << ", ";
        }
        else
    ...
    for ( std::size_t i = 0; i < (tf_number * DEFAULT_NBINS); i++ )
    {
        if ( (i % DEFAULT_NBINS) == 0 )
        {
            std::cout << "m_o_histogram[" << (i / DEFAULT_NBINS) << "]:";
        }
        
        std::cout << m_o_histogram[i];
