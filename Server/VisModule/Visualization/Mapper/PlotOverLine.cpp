    ...
    for( std::size_t K = 0; K < nz; K++ )
    {
        for( std::size_t J = 0; J < ny; J++ )
        {
            for( std::size_t I = 0; I < nx; I++ )
            {
                ...
                scalar[1] = m_values[m_plot_variable][index( I  , J  , K+1 )];
                scalar[2] = m_values[m_plot_variable][index( I+1, J  , K+1 )];
                scalar[3] = m_values[m_plot_variable][index( I+1, J+1, K+1 )];
                scalar[4] = m_values[m_plot_variable][index( I  , J+1, K   )];
                scalar[5] = m_values[m_plot_variable][index( I  , J  , K   )];
                scalar[6] = m_values[m_plot_variable][index( I+1, J  , K   )];
                scalar[7] = m_values[m_plot_variable][index( I+1, J+1, K   )];   
...
void PlotOverLine::for_tetrahedral_mesh( const vismodule::Vec3 P0, const vismodule::Vec3 P1 )
{
    ...
    for( int cell = 0; cell < ncells; cell++, id+=4 )
    {
        ...
        vismodule::Vec4 scalar(
            m_values[m_plot_variable][ local_id[0] ],
            m_values[m_plot_variable][ local_id[1] ],
            m_values[m_plot_variable][ local_id[2] ],
            m_values[m_plot_variable][ local_id[3] ] );
