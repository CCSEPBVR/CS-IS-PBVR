 ...
 *
 *  $Id: MarchingCubes.cpp 631 2010-10-10 02:15:35Z naohisa.sakamoto $
 */
/****************************************************************************/
...
/*==========================================================================*/
template <typename T>
const vismodule::Vector3f MarchingCubes::interpolate_vertex(
    const vismodule::Vector3f& vertex0,
    const vismodule::Vector3f& vertex1 ) const
{
    const T* const values = static_cast<const T*>( BaseClass::m_volume->values().pointer() );
    const vismodule::StructuredVolumeObject* volume =
        reinterpret_cast<const vismodule::StructuredVolumeObject*>( BaseClass::m_volume );
...
/*==========================================================================*/
template <typename T>
const vismodule::RGBColor MarchingCubes::calculate_color( void )
{
        ...
        for ( vismodule::UInt32 y = 0; y < resolution.y(); ++y )
        {
            for ( vismodule::UInt32 x = 0; x < resolution.x(); ++x )
            {
                ...
                if ( x != ncells.x() )
                {
                    ...
                    {
                        const vismodule::Vector3f v1( static_cast<float>(x), static_cast<float>(y), static_cast<float>(z) );
                        const vismodule::Vector3f v2( static_cast<float>(x+1), static_cast<float>(y), static_cast<float>(z) );
                        const vismodule::Vector3f isopoint( this->interpolate_vertex<T>( v1, v2 ) );
                ...
                if ( y != ncells.y() )
                {
                    ...
                    {
                        const vismodule::Vector3f v1( static_cast<float>(x), static_cast<float>(y), static_cast<float>(z) );
                        const vismodule::Vector3f v2( static_cast<float>(x), static_cast<float>(y+1), static_cast<float>(z) );
                        const vismodule::Vector3f isopoint( this->interpolate_vertex<T>( v1, v2 ) );
                ...
                if ( z != ncells.z() )
                {
                    ...
                    {
                        const vismodule::Vector3f v1( static_cast<float>(x), static_cast<float>(y), static_cast<float>(z) );
                        const vismodule::Vector3f v2( static_cast<float>(x), static_cast<float>(y), static_cast<float>(z+1) );
                        const vismodule::Vector3f isopoint( this->interpolate_vertex<T>( v1, v2 ) );
