 ...
 */
void FilterIoPlot3d::plot3d_coordinateBlock_setDimension(Plot3DCoordinateBlock* cBlock, int i, int j, int k) {
 ...
 * @param typeId
 */
void FilterIoPlot3d::plot3d_coordinateBlock_allocateMemory(Plot3DCoordinateBlock* cBlock, bool hasIBlank, DataTypeId typeId) {
 ...
 * @param cBlock Plot3DCoordinateGrid* the coordinate grid instance to work on
 */
void FilterIoPlot3d::plot3d_coordinateBlock_deAllocateMemory(Plot3DCoordinateBlock* cBlock) {
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_coordinateBlock_setDiskByteSize(Plot3DCoordinateBlock* cBlock, GridFile* gFile, bool hasIBlanks) {
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_coordinateBlock_readWhole(Plot3DCoordinateBlock* cBlock, GridFile* gFile, bool hasIBlanks) {
    int err_state = RTC_OK;
    int nPointsInBlock = cBlock->ncoords;
...
//	unsigned char* iBuff = cBlock->ibTuples->ucBuffer;
    unsigned char* iBuff = (unsigned char*)malloc(4 * nPointsInBlock);
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_coordinateBlock_readPlanes(Plot3DCoordinateBlock* cBlock, GridFile* gFile, bool hasIBlanks) {
    int k;
    int status = 0;
 ...
 * @param cBlock Plot3DCoordinateGrid* the coordinate grid instance to work on
 */
void FilterIoPlot3d::plot3d_coordinateBlock_writeToArray(Plot3DCoordinateBlock* cBlock, int offset, int skip) {
    ...
    DataArray_copyAsFloat(&(cBlock->coordinates), info->m_coords, offset, skip);
};
 ...
 */
void FilterIoPlot3d::plot3d_coordinateBlock_dump(Plot3DCoordinateBlock* cBlock, FILE* file) {
    int n;
    double x, y, z;
    int ib = -1;
...
void FilterIoPlot3d::plot3d_coordinateBlock_find_dims(Plot3DCoordinateBlock* cBlock, DataTypeId dataTypeId) {
    ...
    LOGOUT(FILTER_LOG, (char*)"## block =>[ min_x:%+.5lf   min_y:%+.5lf   min_z:%+.5lf]   \n", cBlock->xRange.min, cBlock->yRange.min, cBlock->zRange.min);
    LOGOUT(FILTER_LOG, (char*)"## block =>[ max_x:%+.5lf   max_y:%+.5lf   max_z:%+.5lf]   \n", cBlock->xRange.max, cBlock->yRange.max, cBlock->zRange.max);
}
