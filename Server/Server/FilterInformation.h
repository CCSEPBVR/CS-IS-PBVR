...
public:
    ...
    virtual bool isPbvrFilterInfo() const {return false;}
    virtual bool isPbvrSubvolume(int subvols, int rank, int nprocs) const {
        return (subvols % nprocs == rank);
    }
    ...
    virtual pbvr::filter::DIVISION_TYPE getFilterDivision() const {
        return pbvr::filter::UNKNOWN_DIVISION;
    }
};
...
public:
    ...
    int loadMpiSubvolume(int step) const;
