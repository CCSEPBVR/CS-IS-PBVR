...
FilterInformationList::~FilterInformationList()
{
    ...
    for (itr = m_list.begin(); itr != m_list.end(); ++itr) {
        FilterInformationFile* info = *itr;
...
int FilterInformationList::loadPFL( const std::string& filename )
{
    ...
    {
        FilterInformationFile *fi = new FilterInformationFile();
...
/**
 * フィルタリストに対して、指定ステップ番号の物理量データを読み込みを行う。
 * @param step		ステップ番号(フィルタパラメータファイル記述(start_step〜end_step)
 * @return		読込フィルタ数
 */
int FilterInformationList::loadMpiSubvolume(int step) const
{
 ...
 */
void FilterInformationList::releaseMpiSubvolume() const
{
    if ( m_list.empty() ) return;
