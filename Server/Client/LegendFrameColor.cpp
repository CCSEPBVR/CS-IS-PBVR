...
{
    kvs::visclient::LegendFrameColor* m_editor;

public:

    CancelButton( kvs::visclient::LegendFrameColor* editor ) :
...
public:
    ...
    SaveButton( kvs::visclient::LegendFrameColor* editor ) :
        kvs::glut::PushButton( editor ),
        m_editor( editor )
    {
    }
...
LegendFrameColor::LegendFrameColor( kvs::visclient::LegendpanelUI* parent, kvs::ScreenBase* parent_screen ) :
    m_parent( parent ),
    ...
    m_visible( false )
{
    const std::string title = "Color Map Editor(Frameline)";
    const int x = 905;
    const int y = 105;
    const int width = 350;
    ...
    m_color_palette->setCaption( "Color palette(Frameline)" );
...
kvs::ScreenBase* LegendFrameColor::screen()
{
    return m_screen;
}

const kvs::glut::ColorPalette* LegendFrameColor::colorPalette() const
{
    return m_color_palette;
}

const kvs::RGBColor LegendFrameColor::color() const
{
    return m_color_palette->color();

}

void LegendFrameColor::cancel()
{
    this->hide();
}

void LegendFrameColor::save()
{
//  m_parent->setFranelineColor();
}
