...
int filter_io_open_DataFile(GridFile* gFile, const char* filepath, ReadMode rMode) {
    ...
    if (!gFile->handle) {
	filter_log(FILTER_ERR, "##filter_io_reader cannot open data file %s \n", filepath);
	return RTC_NG;
    }
...
int filter_io_checkFileEndian(GridFile* gFile) {
	...
	filter_log(FILTER_ERR,"ERROR: File Endian can not be determined\n");
	return -1;
    }
 ...
 */
static void filter_io_swap_bufferEndian(void* var, int size, int count) {
    unsigned char* tmp0 =malloc(size*count);//[size * count];
    unsigned char* tmp1 =malloc(size*count);//[size * count];
    int i, c;
 ...
 * @return status, error code - 0 for OK, -1 for error
 */
static inline int filter_io_readBuffer_ASCII(GridFile* gFile, char* buffer, const char* format, int size, int count) {
    int status = 0;
    int index = 0;
	...
	if (fscanf(fh, format, &(buffer[index * size]))) {
	    ...
	    filter_log(FILTER_LOG, "##WARN: break at index %i in filter_io_readBuffer_ASCII\n", index);
	    break;
	}
    }
 ...
 * @return status, error code - 0 for OK, -1 for error
 */
static inline int filter_io_readBuffer_Binary(GridFile* gFile, void* buffer, int size, Int64 count) {
    ...
    if (count != objCount) {
	filter_log(FILTER_ERR, "##ERROR: filter_io_readBuffer - Expected %ld objects of size %d, but was only able to read %ld \n", count, size, objCount);
	return RTC_NG;
    } else {
 ...
 * @return status, error code - 0 for OK, -1 for error
 */
static inline int filter_io_readObject_Binary(GridFile* gFile, void* var, int size) {
    int status = 0;
    FILE* fh = gFile->handle;
    ...
    if (objCount != 1) {
	filter_log(FILTER_ERR, "##ERROR: readTuple - Expected 1 object of size %i, but was only able to read %ld \n", size, objCount);
	status = -1;
    }
    return status;
}
 ...
 * @return status, error code - 0 for OK, -1 for error
 */
static inline int filter_io_readObject_ASCII(GridFile* gFile, void* var, const char* format) {

    FILE* fh = gFile->handle;
    DEBUG(3,"##Reading Object from file using format string: '%s' \n", format);
    int objCount = fscanf(fh, format, var);
    if (objCount != 1) {
	filter_log(FILTER_ERR, "##ERROR: readTuple - Expected 1 object of type %s, but was only able to read %i \n", format, objCount);
	return RTC_NG;
    }
    return RTC_OK;
}
 ...
 * @return status, error code - 0 for OK, -1 for error
 */
int filter_io_readObject(GridFile* gFile, void* var, DataTypeId typeId) {
    int status = RTC_OK;
    if (gFile->rMode == ASCII) {
...
int filter_io_readBuffer(GridFile* gFile, void* var, Int64 count, DataTypeId typeId) {
	...
	const char* format = dataTypes[typeId].scanFormat;
	err_state = filter_io_readBuffer_ASCII(gFile, var, format, size, count);
    } else {
	int size = dataTypes[typeId].onDiskSize;
...
Int64 filter_io_open_FortranBlock(GridFile * gFile) {
	...
	filter_log(FILTER_ERR,"##ERROR:!! FBLOCK ALREADY OPEN!!!\n");
	exit(0);
    }
    return blockSize;
}
...
void filter_io_close_FortranBlock(GridFile * gFile) {
	...
	filter_log(FILTER_ERR, "##ERROR: !! FBLOCK: SIZE INCORRECT !!( End at %ld, but expected:%ld)\n", loc, gFile->OpenFblockEnd);
