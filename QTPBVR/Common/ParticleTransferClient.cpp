...
int jpv::ParticleTransferClient::sendMessageCS( const ParticleTransferClientMessage& message )
{
    int size = message.byteSizeCS();
    //#ifdef WIN32
...
int jpv::ParticleTransferClient::sendMessageIS( const ParticleTransferClientMessage& message )
{
    int size = message.byteSizeIS();
//#ifdef WIN32
//	char* buf = new char [size+1];
//#else
    char* buf = new char [size];
...
int jpv::ParticleTransferClient::recvMessageCS( ParticleTransferServerMessage* message )
{
    // サーバーメッセージの受信
    ...
    message->unpack_messageCS( ss.str().c_str() );
...
int jpv::ParticleTransferClient::recvMessageIS( ParticleTransferServerMessage* message )
{
    // サーバーメッセージの受信
//    const size_t hSize = sizeof( message->m_header ) + sizeof( message->m_message_size );
    const size_t hSize = sizeof( message->m_header ) + sizeof( message->m_message_size );
...
// MODIFIED END   Fj 2015.03.04
    ...
    message->unpack_messageIS( ss.str().c_str() );
    //MOVED FROM 181)T0603 2020.03.05
    fprintf( stdout, "message->m_time_step : %d\n",  message->m_time_step );
    std::cout << "Receive Particle Size = " << message->m_number_particle << std::endl;
//  }
