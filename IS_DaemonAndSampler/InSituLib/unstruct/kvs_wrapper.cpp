...
//    show_timer( time );
    ...
    if (skip_flag)
    {
        ...
        ///--------変動係数配列をファイル出力----------//
        //--------------------------------------//
        kvs::ValueArray<float> coords( particleBase.m_cov_coords );
        kvs::ValueArray<Byte>  colors( particleBase.m_cov_colors );
        kvs::ValueArray<float> normals(particleBase.m_cov_normals );
        ...
        ss << std::setfill('0') << std::setw(7) << num_nodes;
        ss << ".kvsml";
        particleBase.m_covFilePath = particleBase.m_visParamDir + "particle_out/cov_";
        particleBase.m_covFilePath += ss.str();
        // 20181226 end
#endif
        ...
        if( new_rank == 0 )
        {
            ...
            if (async_io_enabled){
                ...
                particle_write_thread->join(true);
                particle_write_thread->setPointObject( point_object );
                particle_write_thread->setFilename(particleBase.m_covFilePath.c_str());
                particle_write_thread->setTimestep(time_step ,particleBase.m_stateFilePath.c_str());
                particle_write_thread->setStartTimestep(st_time_step); //add by shimomura 20240808
                particle_write_thread->work(true);
            }// If async_io is disabled, use kvs::PointExporter here in main thread.
                ...
                kvs::KVSMLObjectPoint* kvsml_object = new kvs::PointExporter<kvs::KVSMLObjectPoint>( point_object );
                kvsml_object->setWritingDataType( kvs::KVSMLObjectPoint::ExternalBinary );
                kvsml_object->write( particleBase.m_covFilePath.c_str() );
                delete kvsml_object;
            }
            delete point_object;
        }
    ...
    pbvr_parameters& particleBase)
{
    ...
    std::vector<kvs::Real32>& vertex_normals_x = ens_param.vertex_normals_x;
    std::vector<kvs::Real32>& vertex_normals_y = ens_param.vertex_normals_y;
    std::vector<kvs::Real32>& vertex_normals_z = ens_param.vertex_normals_z;
    ...
    std::vector<kvs::Real32> per_thread_normals_x[max_threads];
    std::vector<kvs::Real32> per_thread_normals_y[max_threads];
    std::vector<kvs::Real32> per_thread_normals_z[max_threads];
...
//                        local_coord_array[p_id] = kvs::Vector3f (0,0,0);
                       ...
                       if(p_id == SIMD_BLK_SIZE )
                       {
                           ...
                           size_t base_n = per_thread_normals_x[thid].size();
...
//                           per_thread_normals[thid].resize(base_n + p_id * 3);
                           per_thread_normals_x[thid].resize(base_n + p_id );
                           per_thread_normals_y[thid].resize(base_n + p_id );
                           per_thread_normals_z[thid].resize(base_n + p_id );
...
//                               per_thread_normals[thid][n+2] = grad_array_z[j];
                               per_thread_normals_x[thid][n] = grad_array_x[j];
                               per_thread_normals_y[thid][n] = grad_array_y[j];
                               per_thread_normals_z[thid][n] = grad_array_z[j];
...
const size_t base_normals  = vertex_normals_x.size();
...
for (int t = 0; t < max_threads; ++t) 
{
    ...
    add_normals += per_thread_normals_x[t].size();
    add_cellids += per_thread_cellids[t].size();
//    std::cout << mpi_rank <<  ": per_thread_scalars[t].size() = " << per_thread_scalars[t].size() << ", nparticles[t] = "  << nparticles[t] << std::endl;
}
...
vertex_normals_x.resize(base_normals + add_normals);
vertex_normals_y.resize(base_normals + add_normals);
vertex_normals_z.resize(base_normals + add_normals);
...
for (int t = 0; t < T; ++t) {
    ...
    sz_n[t]  = per_thread_normals_x[t].size();   // x/y/z は同サイズ前提（違うなら別々に）
...
//        }
    ...
    {
        const auto& srcx = per_thread_normals_x[t];
        const auto& srcy = per_thread_normals_y[t];
        const auto& srcz = per_thread_normals_z[t];
//        if (!srcx.empty()) std::copy(srcx.begin(), srcx.end(), vertex_normals_x.begin() + off_n[t]);
//        if (!srcy.empty()) std::copy(srcy.begin(), srcy.end(), vertex_normals_y.begin() + off_n[t]);
//        if (!srcz.empty()) std::copy(srcz.begin(), srcz.end(), vertex_normals_z.begin() + off_n[t]);
         std::copy(srcx.begin(), srcx.end(), vertex_normals_x.begin() + off_n[t]);
         std::copy(srcy.begin(), srcy.end(), vertex_normals_y.begin() + off_n[t]);
         std::copy(srcz.begin(), srcz.end(), vertex_normals_z.begin() + off_n[t]);
    }
    // cellids
    {
...
//size_t off_nx = base_normals;
//size_t off_ny = base_normals;
//size_t off_nz = base_normals;
...
//    const auto& src_nx = per_thread_normals_x[t];
//    const auto& src_ny = per_thread_normals_y[t];
//    const auto& src_nz = per_thread_normals_z[t];
...
////    off_n += src_n.size();
//    std::copy(src_nx.begin(), src_nx.end(), vertex_normals_x.begin() + off_nx);
//    off_nx += src_nx.size();
//    std::copy(src_ny.begin(), src_ny.end(), vertex_normals_y.begin() + off_ny);
//    off_ny += src_ny.size();
//    std::copy(src_nz.begin(), src_nz.end(), vertex_normals_z.begin() + off_nz);
//    off_nz += src_nz.size();
...
//    MPI_Allreduce(&local_size, &recv_size, 1, MPI_INT, MPI_MIN, MPI_COMM_WORLD); 
    ...
    std::vector<float>& vertex_normals_x = ens_param.vertex_normals_x;
    std::vector<float>& vertex_normals_y = ens_param.vertex_normals_y;
    std::vector<float>& vertex_normals_z = ens_param.vertex_normals_z;
...
//    std::vector<float> recv_normals(normals_counts[row_rank]);
    std::vector<float> recv_normals_x(scalars_counts[row_rank]);
    std::vector<float> recv_normals_y(scalars_counts[row_rank]);
    std::vector<float> recv_normals_z(scalars_counts[row_rank]);
...
//    std::vector<float> tmp_term(3*recv_size);
    std::vector<float> tmp_term_x(recv_size);
    std::vector<float> tmp_term_y(recv_size);
    std::vector<float> tmp_term_z(recv_size);
//    std::vector<float> varience_normals(normals_counts[row_rank]);
    std::vector<float> varience_normals_x(scalars_counts[row_rank]);
    std::vector<float> varience_normals_y(scalars_counts[row_rank]);
    std::vector<float> varience_normals_z(scalars_counts[row_rank]);
        // 二乗
...
//    std::vector<float> recv_tmp_term(normals_counts[row_rank]);
    std::vector<float> recv_tmp_term_x(scalars_counts[row_rank]);
    std::vector<float> recv_tmp_term_y(scalars_counts[row_rank]);
    std::vector<float> recv_tmp_term_z(scalars_counts[row_rank]);
...
//        tmp_term[3*i+2] = vertex_scalars[i] * vertex_normals[3*i+2];
        tmp_term_x[i] = vertex_scalars[i] * vertex_normals_x[i]; 
        tmp_term_y[i] = vertex_scalars[i] * vertex_normals_y[i];  
        tmp_term_z[i] = vertex_scalars[i] * vertex_normals_z[i];
    }
...
//            MPI_SUM, comm_row_vector);
    ...
    // normal_y 
    MPI_Reduce_scatter(vertex_normals_y.data(), recv_normals_y.data(), scalars_counts.data(), MPI_FLOAT,
            MPI_SUM, comm_row_vector);
    
    // normal_z 
    MPI_Reduce_scatter(vertex_normals_z.data(), recv_normals_z.data(), scalars_counts.data(), MPI_FLOAT,
            MPI_SUM, comm_row_vector);
...
//            MPI_SUM, comm_row_vector);
    ...
    // tmp_term_x
    MPI_Reduce_scatter(tmp_term_x.data(), recv_tmp_term_x.data(), scalars_counts.data(), MPI_FLOAT,
            MPI_SUM, comm_row_vector);
    
    // tmp_term_y
    MPI_Reduce_scatter(tmp_term_y.data(), recv_tmp_term_y.data(), scalars_counts.data(), MPI_FLOAT,
            MPI_SUM, comm_row_vector);

    // tmp_term_z
    MPI_Reduce_scatter(tmp_term_z.data(), recv_tmp_term_z.data(), scalars_counts.data(), MPI_FLOAT,
            MPI_SUM, comm_row_vector);
    timer.stop();
    timeN[3] += timer.sec();
    timer.start();
...
#pragma simd
    for (int i =0 ; i< scalars_counts[row_rank] ; i++ )
    {
        ...
        recv_normals_x[i]  *= -mpi_size_inv;
        recv_normals_y[i]  *= -mpi_size_inv;
        recv_normals_z[i]  *= -mpi_size_inv;
        recv_tmp_term_x[i] *= mpi_size_inv; 
        recv_tmp_term_y[i] *= mpi_size_inv; 
        recv_tmp_term_z[i] *= mpi_size_inv; 
    }
...
//    ens_param.average_normals = recv_normals;     
    ens_param.average_normals_x = recv_normals_x;     
    ens_param.average_normals_y = recv_normals_y;     
    ens_param.average_normals_z = recv_normals_z;     
    ens_param.varience_scalars= varience    ;
//    ens_param.varience_normals= varience_normals;
    ens_param.varience_normals_x= varience_normals_x;
    ens_param.varience_normals_y= varience_normals_y;
    ens_param.varience_normals_z= varience_normals_z;
...
//    auto tf_v = particleBase.m_tf[1];
    auto tf_v = particleBase.m_tf[0];
    ...
    std::vector<float>& average_normals_x = ens_param.average_normals_x;
    std::vector<float>& average_normals_y = ens_param.average_normals_y;
    std::vector<float>& average_normals_z = ens_param.average_normals_z;

    std::vector<float>& varience_scalars = ens_param.varience_scalars;
    std::vector<float>& varience_normals = ens_param.varience_normals;
    std::vector<float>& varience_normals_x = ens_param.varience_normals_x;
    std::vector<float>& varience_normals_y = ens_param.varience_normals_y;
    std::vector<float>& varience_normals_z = ens_param.varience_normals_z;
    //棄却法を適応する
#if 1
...
//            co_varietion[ i ]   =  std::sqrt(varience_scalars[ i ] )/ vertex_scalars[i];
//        else
...
#pragma omp simd
        ...
        for( int j = 0; j < remain_BLK; j++ ) 
        {
            opacity_array_average [j] = tf_a.opacityMap().at(average_scalars[ i+j ]);
            //                    opacity_array_average [j] = tf.opacityMap().at(vertex_scalars[ i+j ]);
            opacity_array_varience[j] = tf_v.opacityMap().at(varience_scalars[ i+j ]);
            //                    対数値を参照
...
#pragma omp simd
        ...
        for( int j = 0; j < remain_BLK; j++ ) 
        {
            ...
            if ( density_array_average[j] > max_density * R )
            {
                th_timer.start();
                // Calculate a color.
                const kvs::RGBColor color( tf_a.colorMap().at( average_scalars[ i+j ] ) );
...
//                   const kvs::RGBColor color( tf.colorMap().at( log_vertex_scalars[ i+j ] ) );
                ...
                th_vertex_normals.push_back( average_normals_x[i+j] );
                th_vertex_normals.push_back( average_normals_y[i+j] );
                th_vertex_normals.push_back( average_normals_z[i+j] );
            ...
            if ( density_array_varience[j] > max_density * R )
            {
                th_timer.start();
                // Calculate a color.
                const kvs::RGBColor color( tf_v.colorMap().at( varience_scalars[ i+j ] ) );
                //                    対数値を参照
                //                   const kvs::RGBColor color( tf.colorMap().at( log_vertex_scalars[ i+j ] ) );
                th_timer.stop();
                th_timeN[10] += th_timer.sec();
...
//                th_vertex_varience_normals.push_back( varience_normals[3*(i+j)+2] );
                th_vertex_varience_normals.push_back( varience_normals_x[i+j] );
                th_vertex_varience_normals.push_back( varience_normals_y[i+j] );
                th_vertex_varience_normals.push_back( varience_normals_z[i+j] );
...
//    if (repetitions < ens_number ) repetitions = 1;
    ...
    uniform_sampling(cell, 
                     ...
                     particleBase); 
    timer.stop();
//    std::cout << mpi_rank << ", uniform_sampling = " << timer.sec() << std::endl;  
    timer.start();
    ...
    ensemble_reduce_scatter(cell, 
            ...
            particleBase);
    timer.stop();
//    std::cout << mpi_rank << ", ensemble_reduce_scatter = " << timer.sec() << std::endl;  
    timer.start();

    rejection_process(cell, ens_param, particleBase);
    timer.stop();
//    std::cout << mpi_rank << ", rejection_process = " << timer.sec() << std::endl;  
}
...
//    std::cout << mpi_rank << ", uniform_sampling = " << timer.sec() << std::endl;  
    ...
    timer.stop();
//    std::cout << mpi_rank << ", ensemble_reduce = " << timer.sec() << std::endl;  
    timer.start();

    if(mpi_rank < MPIprocess_per_ensemble) rejection_process(cell, ens_param, particleBase);
    timer.stop();
//    std::cout << mpi_rank << ", ensemble_reduce = " << timer.sec() << std::endl;  
}
...
void calculation_glad(const int nparticles_count, const int nvariables,
        ...
        const std::vector< pbvr::CellBase<Type>* > interp, 
        const kvs::Vector3f* local_coord_array, 
        const kvs::UInt32* cell_index,
        float* grad_array_x, 
        float* grad_array_y, 
        float* grad_array_z)
{
...
void calc_histogram(const int tf_number,const int nvariables, const int ncells , const int ens_number,
        TransferFunctionSynthesizer** th_tfs, 
        std::vector< std::vector<pbvr::TransferFunction>>& th_tf, 
        const std::vector< std::vector< pbvr::CellBase<Type>* > > cell, 
        const kvs::ValueArray<float> o_min,//TFSから読み込む最大最小値
        const kvs::ValueArray<float> o_max,
        const kvs::ValueArray<float> c_min,
        const kvs::ValueArray<float> c_max,
        pbvr_parameters& particleBase)
...
#pragma omp parallel
    {    
        ...
        std::vector<float> th_o_scalars_array[ SIMD_BLK_SIZE ];
        std::vector<float> th_c_scalars_array[ SIMD_BLK_SIZE ];

        for (int i = 0; i < SIMD_BLK_SIZE; i++ )
        {
            th_o_scalars_array[i].resize( tf_number );
            th_c_scalars_array[i].resize( tf_number );
        }
...
#pragma omp for schedule( dynamic ) nowait
        for ( size_t index = 0; index < ncells; index += SIMD_BLK_SIZE )
        {
            ...
            {
                cell_index[cell_BLK] = (kvs::UInt32)(index + cell_BLK);
                local_center_array[cell_BLK] = cell[thid][0]->localGravityPoint();
            }

            for(int i = 0; i < nvariables; i++)
            {
                cell[thid][i]->bindCellArray(remain, cell_index);
            }

            //  hhistgram算出
            cell[thid][0]->setLocalPointArray( remain, local_center_array );
            cell[thid][0]->transformLocalToGlobalArray( remain,
                    local_center_array,
                    global_center_array );

            th_tfs[thid]->SynthesizedOpacityScalarsArray( cell[thid],
                    remain,
                    local_center_array,
                    global_center_array,
                    th_o_scalars_array );

            th_tfs[thid]->SynthesizedColorScalarsArray( cell[thid],
                    remain,
                    local_center_array,
                    global_center_array,
                    th_c_scalars_array );
    ...
    //     std::cout << "vertex_scalars.size() = " << vertex_scalars.size() << ", recv_scalars = " << recv_scalars.size() << ", local_size = " << local_size <<std::endl;
    // MPI_reduce による変数の合算
    MPI_Reduce(o_scalars_array.data(), recv_o_scalars.data(), tf_number*ncells, MPI_FLOAT,
            MPI_SUM, 0, comm_row_vector);
    ...
    if(k_col == 0)
    {
        // 平均値計算
        //const float mpi_size_inv =float( ens_param.MPIprocess_per_ensemble) /float(mpi_size);
        const float mpi_size_inv =1.f/float(row_size);

#pragma simd 
        for (int i =0 ; i< ncells*tf_number ; i++ )
        {
            recv_o_scalars[i]    *= mpi_size_inv;
            recv_c_scalars[i]    *= mpi_size_inv;
            //            recv_sq_scalars[i] *= mpi_size_inv;
        }
                         ...
                         unsigned int* connections, int ncells, const pbvr::VolumeObjectBase::CellType& celltype, pbvr_parameters& particleBase) //celltype  enum 型に変更
{
    ...
    // 分散値データ
    std::vector<float> tmp_varience;
    std::vector<float> tmp_varience_normals;
    std::vector<float> varience;
    std::vector<float> varience_coords;
    std::vector<float> varience_colors;
    std::vector<float> varience_normals;
    std::vector<float> cov_coords;
    std::vector<float> cov_colors;
    std::vector<float> cov_normals;

//  pbvr::cellbaseの宣言 
//    std::vector< pbvr::CellBase<Type>* > cell;
    std::vector< std::vector< pbvr::CellBase<Type>* > >  cell;
        ...
        case pbvr::VolumeObjectBase::Tetrahedra:
            {
                ...
                for ( int i = 0; i < max_threads; i++ )
                {
                    cell[ i ].resize( nvariables );
                    for ( int j = 0; j < nvariables; j++ )
                    {
                        cell[i][j]  = new pbvr::TetrahedralCell<Type>( values[j], coordinates, ncoords, connections, ncells );
                    }
        ...
        case pbvr::VolumeObjectBase::Hexahedra:
            {
                ...
                for ( int i = 0; i < max_threads; i++ )
                {
                    ...
                    for ( int j = 0; j < nvariables; j++ )
                    {
                        cell[i][j]  = new pbvr::HexahedralCell<Type>( values[j], coordinates, ncoords, connections, ncells );
                    }
...
//  .tfファイルを参照
    ...
    auto tf_var = particleBase.m_tf[0];
...
#define OLD_ENSEMBLE
    ...
    // ヒストグラム, minmax計算
    calc_histogram(tf_number, nvariables, ncells, ens_number, th_tfs, th_tf, cell, 
            c_min, c_max, c_min, c_max, particleBase);
...
//            #pragma omp simd
            ...
            cell[thid][0]->volumeArray( remain, volume_array);
...
//                        local_coord_array[p_id] = kvs::Vector3f (0,0,0);
                       ...
                       if(p_id == SIMD_BLK_SIZE )
                       {
                           ...
                           for( int k = 0; k < nvariables; k++ )
                           {
                                cell[thid][k]->bindCellArray( p_id, cell_index );
                           }
...
// 多変量の計算
                           ...
                           th_tfs[thid]->CalculateScalarsArray( cell[thid],
                                   p_id,
                                   local_coord_array,
                                   global_coord_array,
                                   th_tf[thid],
...
//                    cell[thid][0]->bindCellArray( p_id, cell_index );
                    th_timer.stop();
                    th_timeN[6] += th_timer.sec();
                    th_timer.start();
                    cell[thid][0]->setLocalPointArray(  p_id, local_coord_array );
...
//                    cell[thid][0] -> scalar_ary( scalar_array, p_id);
                    th_timer.stop();
                    th_timeN[9] += th_timer.sec();
                    th_timer.start();
                           calculation_glad(p_id, nvariables, th_tfs[thid], th_tf[thid], cell[thid], local_coord_array, cell_index, grad_array_x, grad_array_y, grad_array_z);
//                    cell[thid][0] -> grad_ary( grad_array_x, grad_array_y, grad_array_z, p_id);
...
//            timer.start();
       ...
       std::vector<int> send_buff_int;
        ...
        // 二乗
        std::vector<float> tmp_term(3*local_size);
        std::vector<float> sq_scalars(local_size);
...
//    std::cout << mpi_rank << ", " <<  __LINE__ <<std::endl;
//    std::cout << mpi_rank << " send_to = " <<  send_to << ", recv_from = " << recv_from << ", recv_size = " << recv_size << ",  send_size = " <<  send_size << ", v_coords [cur] = " << v_coords[cur].size() << std::endl;
//std::cout
//  << mpi_rank
//  << " send_size=" << send_size
...
//            #pragma omp simd
                ...
                for( int k = 0; k < nvariables; k++ )
                {
                    cell[thid][k]->bindCellArray( remain_BLK, cell_index );
                }
                ...
                th_timer.start();
                    ...
                    float recv_tmp_term_x[remain_BLK];
                    float recv_tmp_term_y[remain_BLK];
                    float recv_tmp_term_z[remain_BLK];
            ...
            #pragma omp simd
                    ...
                    for( int j = 0; j < remain_BLK; j++ ) 
                    {
                        ...
                        recv_tmp_term_x[j] = recv_tmp_term[3*(i+j)];
                        recv_tmp_term_y[j] = recv_tmp_term[3*(i+j)+1];
                        recv_tmp_term_z[j] = recv_tmp_term[3*(i+j)+2];
                    }
            ...
            #pragma omp simd
                    for( int j = 0; j < remain_BLK; j++ ) 
                    {
                        ...
                        recv_sq_scalars[i+j]    = recv_sq_scalars[i+j] + scalar_array[j] * scalar_array[j]; 
                        recv_tmp_term_x[j]    = recv_tmp_term_x[j] + scalar_array[j] * grad_array_x[j]; 
                        recv_tmp_term_y[j]    = recv_tmp_term_y[j] + scalar_array[j] * grad_array_y[j]; 
                        recv_tmp_term_z[j]    = recv_tmp_term_z[j] + scalar_array[j] * grad_array_z[j]; 
                    }
                    for( int j = 0; j < remain_BLK; j++ ) 
                    {
                        ...
                        recv_tmp_term[3*(i+j)]   = recv_tmp_term_x[j];
                        recv_tmp_term[3*(i+j)+1] = recv_tmp_term_y[j];
                        recv_tmp_term[3*(i+j)+2] = recv_tmp_term_z[j];
                    }
                th_timer.stop();
                th_timeN[6] += th_timer.sec();
                th_timer.start();
...
//            MPI_Waitall(6, req_send, MPI_STATUSES_IGNORE);
        ...
        tmp_term      .swap(v_tmp[cur]);
            ...
            timer.start();
       //　平均,分散値の計算
       tmp_varience.resize(vertex_scalars.size()); 
       tmp_varience_normals.resize(3*vertex_scalars.size()); 
...
#pragma omp simd
       for (int i = 0; i < tmp_term.size(); i++ )
        {
            tmp_term [ i ] =  -2.f* invert_num* tmp_term [ i ] ; 
...
#pragma omp simd
       for (int i = 0; i < vertex_scalars.size(); i++ )
       {
           ...
           tmp_varience_normals[3*i+0]  = tmp_term [ i ] - (-2.f* vertex_scalars[ i ]* vertex_normals[3*i+0]);
           tmp_varience_normals[3*i+1]  = tmp_term [ i ] - (-2.f* vertex_scalars[ i ]* vertex_normals[3*i+1]);
           tmp_varience_normals[3*i+2]  = tmp_term [ i ] - (-2.f* vertex_scalars[ i ]* vertex_normals[3*i+2]);
       }
...
//    kvs::MersenneTwister MT( vertex_scalars.size() +  thid + mpi_rank * nthreads );
    ...
    std::vector<kvs::Real32> th_vertex_varience_normals;
    std::vector<kvs::UInt8>  th_vertex_varience_colors;
    std::vector<kvs::Real32> th_vertex_cov_coords;
    std::vector<kvs::Real32> th_vertex_cov_normals;
    std::vector<kvs::UInt8>  th_vertex_cov_colors;
    std::vector<kvs::Real32>  th_vertex_scalars;
    std::vector<int>  th_vertex_cellids;
    ...
    float opacity_array_varience[ SIMD_BLK_SIZE ];
    float density_array_varience[ SIMD_BLK_SIZE ];
    float opacity_array_cov[ SIMD_BLK_SIZE ];
    float density_array_cov[ SIMD_BLK_SIZE ];
    float th_timeN[20]= {0};
    kvs::Timer th_timer( kvs::Timer::Start );
...
//                    opacity_array[j] = tf.opacityMap().at(vertex_scalars[ i+j ]);
                    ...
                    opacity_array_cov[j]      = tf_v.opacityMap().at(co_varietion[ i+j ]);
//                    対数値を参照
...
#pragma omp simd
                for( int j = 0; j < remain_BLK; j++ ) 
                {
                    density_array_average [j] = opacity_array_average [j] < max_opacity ? -std::log( 1.0f - opacity_array_average [j] ) * sampling_volume_inverse: max_density;  
                    density_array_varience[j] = opacity_array_varience[j] < max_opacity ? -std::log( 1.0f - opacity_array_varience[j] ) * sampling_volume_inverse: max_density;  
                    density_array_cov[j]      = opacity_array_cov[j]      < max_opacity ? -std::log( 1.0f - opacity_array_cov[j] )      * sampling_volume_inverse: max_density;  
                }
                    th_timer.stop();
                    th_timeN[5] += th_timer.sec();
               ...
               if ( density_array_average[j] > max_density * R )
               {
//                   std::cout << mpi_rank << ", vertex_scalars[ i+j ] = " << vertex_scalars[ i+j ] << std::endl;
//                   std::cout << mpi_rank << ", normal_scalars[ i+j ] = " << vertex_normals[ i+j ] << std::endl;
                th_timer.start();
                   // Calculate a color.
...
//                   const kvs::RGBColor color( tf.colorMap().at( log_vertex_scalars[ i+j ] ) );
               ...
               if ( density_array_varience[j] > max_density * R )
               {
                th_timer.start();
                   // Calculate a color.
//                   std::cout << mpi_rank << ", var_scalars[ i+j ] = " << tmp_varience[ i+j ] << std::endl;
...
//                   const kvs::RGBColor color( tf_var.colorMap().at( log_vertex_scalars[ i+j ] ) );
                ...
                th_timer.start();
                   ...
                   th_vertex_varience_normals.push_back( tmp_varience_normals[3*(i+j)+0] );
               ...
               if ( density_array_cov[j] > max_density * R )
               {
                th_timer.start();
                   const kvs::RGBColor color( tf_v.colorMap().at( co_varietion[ i+j ] ) );
                th_timer.stop();
                th_timeN[7] += th_timer.sec();
                th_timer.start();

                   th_vertex_cov_coords.push_back( global_coord_array[j].x() );
                   th_vertex_cov_coords.push_back( global_coord_array[j].y() );
                   th_vertex_cov_coords.push_back( global_coord_array[j].z() );

                   th_vertex_cov_colors.push_back( color.r() );
                   th_vertex_cov_colors.push_back( color.g() );
                   th_vertex_cov_colors.push_back( color.b() );

                   th_vertex_cov_normals.push_back( tmp_varience_normals[3*(i+j)+0] );
                   th_vertex_cov_normals.push_back( tmp_varience_normals[3*(i+j)+1] );
                   th_vertex_cov_normals.push_back( tmp_varience_normals[3*(i+j)+2] );
...
#pragma omp critical
       {
           ...
           varience_coords.insert (varience_coords.end() , th_vertex_varience_coords.begin() , th_vertex_varience_coords.end());
           ...
           cov_coords.insert (cov_coords.end() , th_vertex_cov_coords.begin() , th_vertex_cov_coords.end());
           cov_colors.insert (cov_colors.end() , th_vertex_cov_colors.begin() , th_vertex_cov_colors.end());
           cov_normals.insert(cov_normals.end(), th_vertex_cov_normals.begin(), th_vertex_cov_normals.end());
       }
                th_timer.stop();
                th_timeN[10] += th_timer.sec();
//#pragma omp critical
...
//    // 平均値データを集約する
    ...
    particleBase.m_varience_coords.insert( particleBase.m_varience_coords.end()  , varience_coords.begin() , varience_coords.end());
    particleBase.m_varience_colors.insert( particleBase.m_varience_colors.end()  , varience_colors.begin() , varience_colors.end());
    particleBase.m_varience_normals.insert(particleBase.m_varience_normals.end() , varience_normals.begin(), varience_normals.end());
    particleBase.m_cov_coords.insert( particleBase.m_cov_coords.end()  , cov_coords.begin() , cov_coords.end());
    particleBase.m_cov_colors.insert( particleBase.m_cov_colors.end()  , cov_colors.begin() , cov_colors.end());
    particleBase.m_cov_normals.insert(particleBase.m_cov_normals.end() , cov_normals.begin(), cov_normals.end());
#endif
