...
size_t jpv::ParticleTransferClientMessage::pack( char* buf ) const
{
    ...
    if ( initParam == 1 )
    {
        ...
        index += jpv::Serializer::write( buf + index, arrow_component[0] ); //add by shimomura 20240116
        index += jpv::Serializer::write( buf + index, arrow_component[1] );
        index += jpv::Serializer::write( buf + index, arrow_component[2] );
        //index += jpv::Serializer::write( buf + index, arrow_component[3] );
        //index += jpv::Serializer::write( buf + index, arrow_component[4] );
        //index += jpv::Serializer::write( buf + index, arrow_component[5] );
        index += jpv::Serializer::write( buf + index, arrow_flag );
        for (int i = 0;i<16 ; i++)
        {
...
size_t jpv::ParticleTransferClientMessage::unpack( const char* buf )
{
    ...
    if ( initParam == 1 )
    {
        ...
        index += jpv::Serializer::read( buf + index, arrow_component[0] ); //add by shimomura 20240116
        index += jpv::Serializer::read( buf + index, arrow_component[1] );
        index += jpv::Serializer::read( buf + index, arrow_component[2] );
        //index += jpv::Serializer::read( buf + index, arrow_component[3] );
        //index += jpv::Serializer::read( buf + index, arrow_component[4] );
        //index += jpv::Serializer::read( buf + index, arrow_component[5] );
	index += jpv::Serializer::read( buf + index, arrow_flag );
	for (int i = 0;i<16 ; i++)
	{
...
void jpv::ParticleTransferClientMessage::show( void ) const
{
    ...
    std::cout<<"arrow_flag = " << arrow_flag <<std::endl; //add by shimomura 20240118 
...
size_t jpv::ParticleTransferServerMessage::pack( char* buf ) const
{
    ...
    index += jpv::Serializer::write( buf + index, arrow_component[0] ); //add by shimomura 20230116 
    index += jpv::Serializer::write( buf + index, arrow_component[1] ); //add by shimomura 20230116 
    index += jpv::Serializer::write( buf + index, arrow_component[2] ); //add by shimomura 20230116 
    //index += jpv::Serializer::write( buf + index, arrow_component[3] ); //add by shimomura 20230116 
    //index += jpv::Serializer::write( buf + index, arrow_component[4] ); //add by shimomura 20230116 
    //index += jpv::Serializer::write( buf + index, arrow_component[5] ); //add by shimomura 20230116 
    index += jpv::Serializer::write( buf + index, arrow_flag );
    for (int i = 0;i<16 ; i++)
    {
        index += jpv::Serializer::write( buf + index, m_glyph_xform[i] );
...
size_t jpv::ParticleTransferServerMessage::unpack_message( const char* buf )
{
    ...
    index += jpv::Serializer::read( buf + index, arrow_component[0] );
    index += jpv::Serializer::read( buf + index, arrow_component[1] );
    index += jpv::Serializer::read( buf + index, arrow_component[2] );
    //index += jpv::Serializer::read( buf + index, arrow_component[3] );
    //index += jpv::Serializer::read( buf + index, arrow_component[4] );
...
void jpv::ParticleTransferServerMessage::show( void ) const
{
    ...
    std::cout<<"arrow_component = (" << arrow_component[0] << ", " << arrow_component[1] << ", " << arrow_component[2] <</* ", " << arrow_component[3] 
             << ", " << arrow_component[4] << ", " << arrow_component[5] <<*/ " )"<<std::endl;
