...
{
    std::cout<<"ParticleTransferClient.cpp:L44"<<std::endl;
...
//  if (th == 0) {
    size_t pack_size = 0;
    pack_size = message.pack( buf );
    std::cout<<"ParticleTransferClient.cpp:L111"<<std::endl;
    std::cout<<"pack_size="<<pack_size<<std::endl;
    std::cout<<"message.byteSize="<<size<<std::endl;
    int send_result=0;
    send_result = send( sock, buf, size, 0 );
