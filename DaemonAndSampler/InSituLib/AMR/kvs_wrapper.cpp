...
void generate_particles( const int time_step,
                         ...
                         const std::vector<float>& cell_length,
                         ...
                         const float** values )
{
    ...
    if(mpi_rank==0) std::cout<<"max_threads="<<max_threads<<std::endl;
    if(mpi_rank==0) std::cout<<"start generate_particles"<<std::endl;
    if(mpi_rank==0) std::cout<<"time_step="<<time_step<<std::endl;
    ...
    //頂点解像度と格子解像度
    const int nx = dom.resolution[0];
    const int ny = dom.resolution[1];
    const int nz = dom.resolution[2];
    const int nl = dom.resolution[3];
    const int nxy   = nx   * ny;
    ...
    const kvs::Vector4ui resol( nx, ny, nz, nl );
    const kvs::Vector3ui resol3D( nx, ny, nz );

    //不透明度ボリュームデータ
    float* volume_opacity = new float[ nx*ny*nz*nl ];
    ...
    for ( int i = 0; i < max_threads; i++ )
    {
        ...
        {
            interp[i][j]  = new TFS::TrilinearInterpolator( values[j], resol3D );
        }
    }

    //ファイルからの読み込み用の伝達関数とTFS
    ...
    //std::cout<<"rank="<<mpi_rank<<",max_nparticles="<<max_nparticles<<std::endl;
    ...
    */
    ...
    if( mpi_rank == 0 ) std::cout<<"start omp paralell"<<std::endl;
    ...
    #pragma omp parallel
    {
        ...
        //各スレッド番号,ランク番号をシードにした乱数生成器
        kvs::MersenneTwister MT( thid + mpi_rank * nthreads );

        if( thid+mpi_rank == 0) std::cout<<"L586"<<std::endl;

        if( thid+mpi_rank == 0){
            std::cout<<"trans func: min="<<th_tf[thid][0].minValue()<<
                ", max="<<th_tf[thid][0].maxValue()<<std::endl;
        }

        //#pragma omp critical
        //{
        //if( mpi_rank == 0 ) std::cout<<"thid="<<thid<<std::endl;
        //}
        // 動的な粒子データ配列
        ...
        for( int l = 0; l < nl; l++ )
        {
            ...
            {
                interp[thid][v]->setLeafIndex( l );
                interp[thid][v]->setCellLength( cell_length[l] );
            }
            ...
            for( int J=0; J<outer_loop; J++ )
            {
                ...
                for( int I=0; I<SIMDW; I++ )
                {
                    ...
                    const float x_g = x_l * d + leaf_min_coord[3*l  ];
                    const float y_g = y_l * d + leaf_min_coord[3*l+1];
                    const float z_g = z_l * d + leaf_min_coord[3*l+2];
        ...
        for( int l = 0; l < nl; l++ )
        {
            ...
            for( int J=0; J<outer_loop; J++ )
            {
                ...
                for( int I = 0; I <SIMDW; I++ )
                {
                    ...
                    {
                        volume_opacity[ vertex_id + l * nxyz ] = vertex_opacity[I];
                    }
                }
            } // end of for outer_loop
        } // end of for l

         // 不透明度用の補間関数を生成
        interp_opacity[thid] = new TFS::TrilinearInterpolator( volume_opacity, resol3D );

        if( thid+mpi_rank == 0) std::cout<<"L697"<<std::endl;
        ...
        for( int l = 0; l < nl; l++)
        {
            ...
            //interp_opacityにリーフIDをセット
            interp_opacity[thid]->setLeafIndex( l );
            interp_opacity[thid]->setCellLength( cell_length[l] );
                ...
                for(int I=0; I<SIMDW+1; I++)
                {
                    ...
                    for(int p=0; p < nparticles_I; p++)
                    {
                        ...
                        const kvs::Vector3f coord_g(
                            coord_l.x() * d + leaf_min_coord[3*l  ],
                            coord_l.y() * d + leaf_min_coord[3*l+1],
                            coord_l.z() * d + leaf_min_coord[3*l+2] );
            ...
            } // end of for J outer_loop
        }// end of for l

        if( thid+mpi_rank == 0) std::cout<<"L857:start delete"<<std::endl;
        delete interp_opacity[thid];

        if( thid+mpi_rank == 0) std::cout<<"L877"<<std::endl;
    ...
    */
    ...
    if( mpi_rank == 0 )
    {
        std::cout<<"L1106:time_step="<< time_step <<std::endl;
