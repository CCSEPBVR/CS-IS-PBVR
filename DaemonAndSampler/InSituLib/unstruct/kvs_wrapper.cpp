...
#endif
{
    ...
    if( celltype == pbvr::VolumeObjectBase::CellType::Hexahedra ) // hex type
    {
        std::vector< std::vector< pbvr::HexahedralCell<Type>* > >  interp;
        ...
        if(mpi_rank==0) std::cout<<"max_vec:"<<max_vec<<std::endl;
        if(mpi_rank==0) std::cout<<"min_vec:"<<min_vec<<std::endl;
...
#pragma omp for schedule( dynamic ) nowait
            ...
            for( int cell_base = 0; cell_base < ncells; cell_base += SIMD_BLK_SIZE )
            {
                ...
                /////////////////////////////// Synthesized~ (), CalculateOpacity() ///////////////////////////////////
                //        std::cout <<  " nparticles_num = " << nparticles_num << std::endl;
                ...
                for( int i = 0; i < nparticles_array[cell_BLK]; i+=SIMD_BLK_SIZE )
                {
                    ...
                    while(1)
                    {
                            local_coord_array[j] = kvs::Vector3f ( RandomSamplingInCube( &MT ) );
