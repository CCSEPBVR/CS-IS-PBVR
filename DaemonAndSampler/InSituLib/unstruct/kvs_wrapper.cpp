...
*/
            ...
            for(int cell_BLK = 0; cell_BLK < remain; cell_BLK++ )
            {
                if( cell_opacity_array[cell_BLK] < 0.01 )
                {
                    nparticles_array[cell_BLK] = 0;
                }
                else
                {
        ...
        /////////////////////////////// CalculateOpacity(), CalculateColor() ///////////////////////////////////
            for(int cell_BLK = 0; cell_BLK < remain; cell_BLK++ )
            {
                ...
                for( int i = 0; i < nparticles_array[cell_BLK]; i+=SIMD_BLK_SIZE )
                {
                    ...
                    {
                        interp[thid][j]->bindCellArray( remain_BLK, cell_index );
                    }

                    interp[thid][0]->setLocalPointArray( remain_BLK, local_coord_array );
                    interp[thid][0]->transformLocalToGlobalArray( remain_BLK,
                                                                  local_coord_array,
                                                                  global_coord_array );
