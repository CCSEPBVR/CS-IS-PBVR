...
public:
    ...
    virtual void CalcScalarGradAveraged(
            const int loop_cnt,
            float *scalar_array,
            float *grad_array_x,
            float *grad_array_y,
...
//    virtual const kvs::Real32 scalar_debug() const;
...
inline void CellBase<T>::setLocalPointArray( const int loop_cnt, const kvs::Vector3f *local_array )
{
    ...
    for( int i = 0; i < loop_cnt; i++)
    {
        ...
        {
              m_interpolation_functions_array[i][j]   = m_interpolation_functions[j];
              m_differential_functions_array[i][j]    = m_differential_functions[j];
              m_differential_functions_array[i][j+1]  = m_differential_functions[j+1];
              m_differential_functions_array[i][j+2]  = m_differential_functions[j+2];
        }
...
//template <typename T>
//inline const kvs::Real32 CellBase<T>::scalar_debug() const
//{
//    const size_t nnodes = m_nnodes;
//    const float* N = m_interpolation_functions;
//    const T* s = m_scalars;
...
//        S += static_cast<kvs::Real32>( N[i] * s[i] );
//    }
//        std::cout << "Scalar_debug " << std::endl;
//        std::cout << "m_interpolation_functions[0] = " << N[0] << "\n"
//                  << "m_interpolation_functions[1] = " << N[1] << "\n"
//                  << "m_interpolation_functions[2] = " << N[2] << "\n"
//                  << "m_interpolation_functions[3] = " << N[3] << std::endl;
//        std::cout << "m_scalars[0] = " << s[0] << "\n"
//                  << "m_scalars[1] = " << s[1] << "\n"
//                  << "m_scalars[2] = " << s[2] << "\n"
//                  << "m_scalars[3] = " << s[3] << std::endl;
//
//
//    return S;
//}
...
template <typename T>
void CellBase<T>::CalcScalarGradAveraged(
    const int loop_cnt,
    float *scalar_array,
    float *grad_array_x,
    float *grad_array_y,
...
////    for( int i = 0; i < loop_cnt; i++ )
////    {
////        m_interpolation_functions = m_interpolation_functions_array[i];
////        m_scalars = m_scalars_array[i];
////        scalar_array[i]  = this -> averagedScalar();
////    }
////    
////    for( int i = 0; i < loop_cnt; i++ )
////    {
////        m_differential_functions = m_differential_functions_array[i];
////        m_vertices = m_vertices_array[i]; 
////        const kvs::Vector3f grad_array = this -> gradient(); 
////        grad_array_x[i]  = grad_array.x();
////        grad_array_y[i]  = grad_array.y();
////        grad_array_z[i]  = grad_array.z();
