...
constexpr int k_after_point_size_wait_ms = 3000;
...
QString PointSizeControlTest::envOrDefault( const char* name, const QString& fallback ) const
{
    const QString value = qEnvironmentVariable( name );
    return value.isEmpty() ? ClientTests::configuredPath( name, repoRootPath(), fallback ) : value;
}

QString PointSizeControlTest::repoRootPath() const
{
...
QString PointSizeControlTest::sourceTreePath( const QString& relative_path_from_repo_root ) const
{
    return QDir::cleanPath( QDir( repoRootPath() ).absoluteFilePath( relative_path_from_repo_root ) );
}

bool PointSizeControlTest::waitForCondition( const std::function<bool()>& condition, int timeout_ms, int interval_ms ) const
{
    QElapsedTimer timer;
    timer.start();
...
void PointSizeControlTest::bringWindowToFront( MainWindow* window ) const
{
...
void PointSizeControlTest::setLineEditText( QLineEdit* line_edit, const QString& text ) const
{
...
void PointSizeControlTest::writeMarkdownReport() const
{
...
void PointSizeControlTest::markStepCompleted( const QString& description )
{
    m_steps.push_back( { description, true } );
    logStep( description );
}

PointSizeControlTest::ClientHandles PointSizeControlTest::resolveClientHandles( MainWindow& window ) const
{
    ...
    handles.point_size_control = window.findChild<PointSizeControl*>();
    ...
    if ( !require( handles.point_size_control != nullptr, "PointSizeControl dock not found" ) ) { return handles; }
    ...
    handles.point_size_double_spin_box = handles.point_size_control->findChild<QDoubleSpinBox*>( "pointSizeDoubleSpinBox" );
    ...
    if ( !require( handles.point_size_double_spin_box != nullptr, "pointSizeDoubleSpinBox not found" ) ) { return handles; }

    return handles;
}

void PointSizeControlTest::connectClient( const ClientHandles& client ) const
{
...
void PointSizeControlTest::configureRemoteVisualization( const ClientHandles& client ) const
{
    bringWindowToFront( client.main_window );
...
void PointSizeControlTest::waitForObjectAndApply( const ClientHandles& client ) const
{
    QVERIFY2(
        waitForCondition(
            [client]()
...
void PointSizeControlTest::clickJumpAndWaitForCompletion( const ClientHandles& client ) const
{
    QVERIFY2(
        waitForCondition(
            [client]()
...
void PointSizeControlTest::setPointSize( const ClientHandles& client, double value ) const
{
    client.point_size_control->show();
    client.point_size_control->raise();
    client.point_size_control->activateWindow();
    QVERIFY2( client.point_size_control->isVisible(), "PointSizeControl did not become visible" );

    client.point_size_double_spin_box->setFocus();
    client.point_size_double_spin_box->setValue( value );
    QCOMPARE( client.point_size_double_spin_box->value(), value );
    QTest::qWait( k_after_point_size_wait_ms );
}

void PointSizeControlTest::initTestCase()
{
    ...
    m_output_dir_path = envOrDefault(
        ...
        ClientTests::datedTestOutputDir( repoRootPath(), date_stamp, QStringLiteral( "PointSizeControlTest" ) ) );
...
void PointSizeControlTest::cleanupTestCase()
{
...
void PointSizeControlTest::performs_point_size_control_scenario()
{
    ...
    markStepCompleted( QStringLiteral( "PlayBackControlToolBar.cpp: m_jump_push_button clicked and waited until it became enabled again." ) );

    saveScreenshot(
        QStringLiteral( "before_point_size_change.png" ),
        QStringLiteral( "Before changing point size" ) );
    markStepCompleted( QStringLiteral( "Captured screenshot before opening Point Size Control." ) );

    setPointSize( client, 0.5 );
    markStepCompleted( QStringLiteral( "PointSizeControl.ui: pointSizeDoubleSpinBox set to 0.5." ) );

    saveScreenshot(
        QStringLiteral( "after_point_size_0_5.png" ),
        QStringLiteral( "After setting point size to 0.5" ) );
    markStepCompleted( QStringLiteral( "Captured screenshot after setting point size to 0.5." ) );

    setPointSize( client, 2 );
    markStepCompleted( QStringLiteral( "PointSizeControl.ui: pointSizeDoubleSpinBox set to 2." ) );

    saveScreenshot(
        QStringLiteral( "after_point_size_2.png" ),
        QStringLiteral( "After setting point size to 2" ) );
    markStepCompleted( QStringLiteral( "Captured screenshot after setting point size to 2." ) );
