...
QString ColorFunctionVariableTest::envOrDefault( const char* name, const QString& fallback ) const
{
    const QString value = qEnvironmentVariable( name );
    return value.isEmpty() ? fallback : value;
}

QString ColorFunctionVariableTest::repoRootPath() const
{
...
QString ColorFunctionVariableTest::sourceTreePath( const QString& relative_path_from_repo_root ) const
{
    return QDir::cleanPath( QDir( repoRootPath() ).absoluteFilePath( relative_path_from_repo_root ) );
}

bool ColorFunctionVariableTest::waitForCondition( const std::function<bool()>& condition, int timeout_ms, int interval_ms ) const
{
    QElapsedTimer timer;
    timer.start();
...
void ColorFunctionVariableTest::bringWindowToFront( MainWindow* window ) const
{
...
void ColorFunctionVariableTest::bringTransferFunctionEditorToFront( TransferFunctionEditor* editor ) const
{
...
void ColorFunctionVariableTest::setLineEditText( QLineEdit* line_edit, const QString& text ) const
{
...
void ColorFunctionVariableTest::closeUnexpectedDialogs( TransferFunctionEditor* editor ) const
{
...
void ColorFunctionVariableTest::setTransferFunctionCount( const ClientHandles& client, int value ) const
{
...
void ColorFunctionVariableTest::setComboBoxIndex( QComboBox* combo_box, int index, const char* widget_name ) const
{
...
void ColorFunctionVariableTest::saveScreenshot( const QString& file_name, const QString& caption )
{
...
void ColorFunctionVariableTest::writeMarkdownReport() const
{
    ...
    stream << "# TransferFunctionEditorTest::ColorFunctionVariableTest\n\n";
...
void ColorFunctionVariableTest::markStepCompleted( const QString& description )
{
    m_steps.push_back( { description, true } );
    logStep( description );
}

ColorFunctionVariableTest::ClientHandles
ColorFunctionVariableTest::resolveClientHandles( MainWindow& window ) const
{
    ...
    handles.color_function_variable_editor_button =
        handles.transfer_function_editor->findChild<QPushButton*>( "colorFunctionVariableEditorPushButton" );
    handles.color_function_variable_line_edit =
    ...
    if ( !require( handles.color_function_variable_editor_button != nullptr, "colorFunctionVariableEditorPushButton not found" ) ) { return handles; }
    if ( !require( handles.color_function_variable_line_edit != nullptr, "colorFunctionVariableLineEdit not found" ) ) { return handles; }

    return handles;
}

void ColorFunctionVariableTest::connectClient( const ClientHandles& client ) const
{
...
void ColorFunctionVariableTest::configureRemoteVisualization( const ClientHandles& client ) const
{
    bringWindowToFront( client.main_window );
...
void ColorFunctionVariableTest::waitForObjectAndApply( const ClientHandles& client ) const
{
    QVERIFY2(
        waitForCondition(
            [client]()
...
void ColorFunctionVariableTest::clickJumpAndWaitForCompletion( const ClientHandles& client ) const
{
    QVERIFY2(
...
QDialog* ColorFunctionVariableTest::findVisibleVariableEditor() const
{
    for ( QWidget* widget : QApplication::topLevelWidgets() )
    {
        auto* dialog = qobject_cast<QDialog*>( widget );
...
void ColorFunctionVariableTest::setVariableEditorRows( QDialog* dialog, const QStringList& variables ) const
{
...
void ColorFunctionVariableTest::editColorVariablesWithDialog(
    ...
    bool accept_dialog )
{
    ...
    QVERIFY2( client.color_function_variable_editor_button->isEnabled(), "colorFunctionVariableEditorPushButton is disabled" );
    ...
    QTest::mouseClick( client.color_function_variable_editor_button, Qt::LeftButton );
    QVERIFY2( dialog_handled, "VariableEditor was not handled" );
    QTest::qWait( k_window_settle_ms );
}

void ColorFunctionVariableTest::captureTransferFunctionEditorState(
    const ClientHandles& client,
    const QString& file_name,
    const QString& caption,
...
void ColorFunctionVariableTest::initTestCase()
{
    ...
    m_output_dir_path = envOrDefault(
        ...
        ClientTests::datedTestOutputDir( repoRootPath(), date_stamp, QStringLiteral( "TransferFunctionEditorTest/ColorFunctionVariableTest" ) ) );
...
void ColorFunctionVariableTest::cleanupTestCase()
{
...
void ColorFunctionVariableTest::color_function_variable()
{
    ...
    QCOMPARE( client.color_function_variable_line_edit->text(), QStringLiteral( "q1" ) );
    ...
    captureTransferFunctionEditorState(
        client,
        QStringLiteral( "04_transfer_function_editor_c1_after_ok.png" ),
        QStringLiteral( "VariableEditor.uiでC1のFunction Variable編集が反映されたことを確認" ) );
    QCOMPARE( client.color_function_variable_line_edit->text(), QStringLiteral( "q1*0.5" ) );
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: C1のFunction Variable編集が反映された状態をスクリーンショット撮影しました。" ) );

    captureTransferFunctionEditorState(
        client,
        QStringLiteral( "05_transfer_function_editor_c2_after_ok.png" ),
        QStringLiteral( "VariableEditor.uiでC2のFunction Variable編集が反映されたことを確認" ),
        1 );
    QCOMPARE( client.color_function_variable_line_edit->text(), QStringLiteral( "q1^2" ) );
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: colorFunctionComboBoxの2番目を選択し、C2のFunction Variable編集が反映された状態をスクリーンショット撮影しました。" ) );
