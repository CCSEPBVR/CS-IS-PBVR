...
QString ColorMinMaxTest::envOrDefault( const char* name, const QString& fallback ) const
{
    const QString value = qEnvironmentVariable( name );
    return value.isEmpty() ? ClientTests::configuredPath( name, repoRootPath(), fallback ) : value;
}

QString ColorMinMaxTest::repoRootPath() const
{
...
QString ColorMinMaxTest::sourceTreePath( const QString& relative_path_from_repo_root ) const
{
    return QDir::cleanPath( QDir( repoRootPath() ).absoluteFilePath( relative_path_from_repo_root ) );
}

bool ColorMinMaxTest::waitForCondition(
...
void ColorMinMaxTest::bringWindowToFront( MainWindow* window ) const
{
...
void ColorMinMaxTest::bringTransferFunctionEditorToFront( TransferFunctionEditor* editor ) const
{
...
void ColorMinMaxTest::setLineEditText( QLineEdit* line_edit, const QString& text ) const
{
...
void ColorMinMaxTest::setDoubleSpinBoxValue( QDoubleSpinBox* spin_box, double value, const char* widget_name ) const
{
...
void ColorMinMaxTest::writeMarkdownReport() const
{
    ...
    stream << "# TransferFunctionEditorTest::ColorMinMaxTest\n\n";
...
void ColorMinMaxTest::markStepCompleted( const QString& description )
{
    m_steps.push_back( { description, true } );
    logStep( description );
}

ColorMinMaxTest::ClientHandles ColorMinMaxTest::resolveClientHandles( MainWindow& window ) const
{
    ...
    handles.color_server_side_min_max_radio =
        handles.transfer_function_editor->findChild<QRadioButton*>( "colorServerSideMinMaxRadioButton" );
    ...
    if ( !require( handles.color_server_side_min_max_radio != nullptr, "colorServerSideMinMaxRadioButton not found" ) ) { return handles; }
...
void ColorMinMaxTest::connectClient( const ClientHandles& client ) const
{
...
void ColorMinMaxTest::configureRemoteVisualization( const ClientHandles& client ) const
{
    bringWindowToFront( client.main_window );
...
void ColorMinMaxTest::waitForObjectAndApply( const ClientHandles& client ) const
{
    QVERIFY2(
        waitForCondition(
            [client]()
...
void ColorMinMaxTest::clickJumpAndWaitForCompletion( const ClientHandles& client ) const
{
    QVERIFY2(
...
void ColorMinMaxTest::applyTransferFunction( const ClientHandles& client ) const
{
...
void ColorMinMaxTest::initTestCase()
{
    ...
    m_output_dir_path = envOrDefault(
        ...
        ClientTests::datedTestOutputDir( repoRootPath(), date_stamp, QStringLiteral( "TransferFunctionEditorTest/ColorMinMaxTest" ) ) );
...
void ColorMinMaxTest::cleanupTestCase()
{
...
void ColorMinMaxTest::color_min_max()
{
    ...
    QVERIFY2( client.color_server_side_min_max_radio->isChecked(), "colorServerSideMinMaxRadioButton is not checked" );
    saveScreenshot(
    ...
    bringTransferFunctionEditorToFront( client.transfer_function_editor );
    setDoubleSpinBoxValue( client.color_user_defined_min_spin_box, 0.2, "colorUserDefinedMinDoubleSpinBox" );
    saveScreenshot(
    ...
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: colorUserDefinedMinDoubleSpinBoxを0.2に設定し、スクリーンショットを撮影しました。" ) );
    applyTransferFunction( client );
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: applyPushButtonを押しました。" ) );
    client.transfer_function_editor->hide();
    ...
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: colorUserDefinedMinDoubleSpinBoxを0.5に設定し、スクリーンショットを撮影しました。" ) );
    applyTransferFunction( client );
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: applyPushButtonを押しました。" ) );
    client.transfer_function_editor->hide();
    ...
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: colorUserDefinedMaxDoubleSpinBoxを0.6に設定し、スクリーンショットを撮影しました。" ) );
    applyTransferFunction( client );
    markStepCompleted( QStringLiteral( "TransferFunctionEditor.ui: applyPushButtonを押しました。" ) );
    client.transfer_function_editor->hide();
