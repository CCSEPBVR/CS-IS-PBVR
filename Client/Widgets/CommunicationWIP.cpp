...
#include "ui_CommunicationWIP.h"

CommunicationWIP::CommunicationWIP( WebSocketPair* websockets, Viz::Mode* vizMode, kvs::qt::jaea::Screen* screen, QWidget* parent )
    : QDockWidget( parent )
    , ui( new Ui::CommunicationWIP )
    ...
    , m_screen( screen )
{
    ...
    connect( ui->localVizRadioButton                , &QRadioButton::clicked                , this, &CommunicationWIP::onModeClicked );
    connect( ui->remoteVizClientServerRadioButton   , &QRadioButton::clicked                , this, &CommunicationWIP::onModeClicked );
    connect( ui->remoteVizInsituRadioButton         , &QRadioButton::clicked                , this, &CommunicationWIP::onModeClicked );

    connect( ui->volumeDataFilePathPushButton       , &QPushButton::clicked                 , this, &CommunicationWIP::onVolumeDataFilePathClicked );
    connect( ui->transferFunctionFilePathPushButton , &QPushButton::clicked                 , this, &CommunicationWIP::onTransferFunctionFilePathClicked );

    connect( ui->connectPushButton                  , &QPushButton::clicked                 , this, &CommunicationWIP::onConnectClicked );
    connect( ui->disconnectPushButton               , &QPushButton::clicked                 , this, &CommunicationWIP::onDisconnectClicked );
    connect( ui->transferOperatorApplyPushButton    , &QPushButton::clicked                 , this, &CommunicationWIP::onTransferOperator );
    connect( ui->chatLineEdit                       , &QLineEdit::returnPressed             , this, &CommunicationWIP::onChatClicked );
    connect( ui->shareViewPushButton                , &QPushButton::clicked                 , this, &CommunicationWIP::onShareView );

    connect( m_web_sockets->binary()                , &QWebSocket::connected                , this, &CommunicationWIP::onBinaryWebsocketConnected );         // 接続成功(バイナリ)
    connect( m_web_sockets->binary()                , &QWebSocket::disconnected             , this, &CommunicationWIP::onBinaryWebsocketDisconnected );      // 接続切断(バイナリ)
    connect( m_web_sockets->binary()                , &QWebSocket::binaryMessageReceived    , this, &CommunicationWIP::onBinaryWebsocketMessageReceived );   // メッセージ受信(バイナリ)
    connect( m_web_sockets->text()                  , &QWebSocket::connected                , this, &CommunicationWIP::onTextWebsocketConnected );           // 接続成功(テキスト)
    connect( m_web_sockets->text()                  , &QWebSocket::disconnected             , this, &CommunicationWIP::onTextWebsocketDisconnected );        // 接続切断(テキスト)
    connect( m_web_sockets->text()                  , &QWebSocket::textMessageReceived      , this, &CommunicationWIP::onTextWebsocketMessageReceived );     // メッセージ受信(テキスト)
}

CommunicationWIP::~CommunicationWIP()
{
    delete ui;
}

void CommunicationWIP::onVRSharePoint( kvs::Real32 CoordArray[ 2 * 3 ], kvs::Real32 DirectionArray[ 3 ] )
{
...
void CommunicationWIP::loadParameter( const QString& filePath )
{
    // TODO:KPI
    qDebug() << __FILE__ << ":" << __func__ << ":" << filePath;
}

void CommunicationWIP::saveParameter( const QString& filePath )
{
    // TODO:KPI
    qDebug() << __FILE__ << ":" << __func__ << ":" << filePath;
}

void CommunicationWIP::websocketConnected()
{
    if( m_web_sockets->isConnected() )
    {
...
void CommunicationWIP::websocketDisconnected()
{
    if( !m_web_sockets->isConnected() )
    {
...
void CommunicationWIP::updateVizMode()
{
...
void CommunicationWIP::Join( const QJsonObject& dataArray )
{
...
void CommunicationWIP::Left( const QJsonObject& dataArray )
{
...
void CommunicationWIP::ID( const QJsonObject& dataArray )
{
...
void CommunicationWIP::Operator( const QJsonObject& dataArray )
{
...
void CommunicationWIP::transferOperator( const QJsonObject& dataArray )
{
...
void CommunicationWIP::chat( const QJsonObject& dataArray )
{
...
void CommunicationWIP::shareView( const QJsonObject& dataArray )
{
...
void CommunicationWIP::sharePoint( const QJsonObject& dataArray )
{
//     int userID = dataArray[QString::fromUtf8( Protocol::Key::UserID )].toInt();    // 着目点共有を行った送信者
//     QString userIDStr = QString::number( userID );
...
kvs::PolygonObject* CommunicationWIP::createArrowGlyph(
...
void CommunicationWIP::convertObjectInfo( const QJsonObject& dataArray )
{
...
void CommunicationWIP::onModeClicked()
{
...
void CommunicationWIP::onVolumeDataFilePathClicked()
{
...
void CommunicationWIP::onTransferFunctionFilePathClicked()
{
...
void CommunicationWIP::onConnectClicked()
{
    if( m_web_sockets->isConnected() )
    {
...
void CommunicationWIP::onDisconnectClicked()
{
    if( !m_web_sockets->isConnected() )
    {
        emit updateStatusBarMessage( "Not connected." );
...
void CommunicationWIP::onTransferOperator()
{
    if( !m_web_sockets->isConnected() )
    {
        emit updateStatusBarMessage( "Not connected." );
...
void CommunicationWIP::onChatClicked()
{
    if( !m_web_sockets->isConnected() )
    {
        emit updateStatusBarMessage( "Not connected." );
...
void CommunicationWIP::onShareView()
{
    if( !m_web_sockets->isConnected() )
    {
        emit updateStatusBarMessage( "Not connected." );
...
void CommunicationWIP::onBinaryWebsocketConnected()
{
    websocketConnected();
}

void CommunicationWIP::onBinaryWebsocketDisconnected()
{
    websocketDisconnected();
}

void CommunicationWIP::onBinaryWebsocketMessageReceived( const QByteArray& binary )
{
    emit unpack( binary );
}

void CommunicationWIP::onTextWebsocketConnected()
{
    websocketConnected();
}

void CommunicationWIP::onTextWebsocketDisconnected()
{
    websocketDisconnected();
}

void CommunicationWIP::onTextWebsocketMessageReceived( const QString& receivedMessage )
{
