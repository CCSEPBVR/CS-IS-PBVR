...
#include "ui_TransferFunctionEditorWIP.h"

TransferFunctionEditorWIP::TransferFunctionEditorWIP( WebSocketPair* websockets, QWidget *parent )
    : QDialog( parent )
    , ui( new Ui::TransferFunctionEditorWIP )
...
TransferFunctionEditorWIP::~TransferFunctionEditorWIP()
{
    delete ui;
}

void TransferFunctionEditorWIP::updateOperatorState( bool operatorState )
{
    m_is_operator = operatorState;
    updateUI();
}

void TransferFunctionEditorWIP::reset()
{
    ui->numberOfTransferFunctionSpinBox->setValue( 0 );
}

void TransferFunctionEditorWIP::loadParameter( const QString& filePath )
{
    // TODO:KPI
    qDebug() << __FILE__ << ":" << __func__ << ":" << filePath;
}

void TransferFunctionEditorWIP::saveParameter( const QString& filePath )
{
    // TODO:KPI
    qDebug() << __FILE__ << ":" << __func__ << ":" << filePath;
}

void TransferFunctionEditorWIP::onReceiveInitializeTransferFunctionParameter(  const QJsonObject& dataArray )
{
...
void TransferFunctionEditorWIP::initialize()
{
    ui->setupUi( this );
    ...
    connect( ui->numberOfTransferFunctionSpinBox        , &QSpinBox::valueChanged           , this, &TransferFunctionEditorWIP::onNumberOfTransferFunctionValueChanged );

    connect( ui->colorSynthesizerLineEdit               , &QLineEdit::textChanged           , this, &TransferFunctionEditorWIP::onColorSynthesizerChanged );
    connect( ui->colorFunctionComboBox                  , &QComboBox::currentIndexChanged   , this, &TransferFunctionEditorWIP::onColorComboBoxChanged );
    connect( ui->colorFunctionVariableLineEdit          , &QLineEdit::textChanged           , this, &TransferFunctionEditorWIP::onColorFunctionVariableChanged );
    connect( ui->colorFunctionVariableEditorPushButton  , &QPushButton::clicked             , this, &TransferFunctionEditorWIP::onColorFunctionVariableEditorClicked );
    connect( ui->colorUserDefinedMinMaxRadioButton      , &QRadioButton::clicked            , this, &TransferFunctionEditorWIP::onColorRangeModeRadioButtonClicked );
    connect( ui->colorServerSideMinMaxRadioButton       , &QRadioButton::clicked            , this, &TransferFunctionEditorWIP::onColorRangeModeRadioButtonClicked );
    connect( ui->colorUserDefinedMinDoubleSpinBox       , &QDoubleSpinBox::valueChanged     , this, &TransferFunctionEditorWIP::onColorUserDefinedMinChanged );
    connect( ui->colorUserDefinedMaxDoubleSpinBox       , &QDoubleSpinBox::valueChanged     , this, &TransferFunctionEditorWIP::onColorUserDefinedMaxChanged );
    connect( ui->colorMapEditPushButton                 , &QPushButton::clicked             , this, &TransferFunctionEditorWIP::onColorMapEditorClicked );

    connect( ui->opacitySynthesizerLineEdit             , &QLineEdit::textChanged           , this, &TransferFunctionEditorWIP::onOpacitySynthesizerChanged );
    connect( ui->opacityFunctionComboBox                , &QComboBox::currentIndexChanged   , this, &TransferFunctionEditorWIP::onOpacityComboBoxChanged );
    connect( ui->opacityFunctionVariableLineEdit        , &QLineEdit::textChanged           , this, &TransferFunctionEditorWIP::onOpacityFunctionVariableChanged );
    connect( ui->opacityFunctionVariableEditorPushButton, &QPushButton::clicked             , this, &TransferFunctionEditorWIP::onOpacityFunctionVariableEditorClicked );
    connect( ui->opacityUserDefinedMinMaxRadioButton    , &QRadioButton::clicked            , this, &TransferFunctionEditorWIP::onOpacityRangeModeRadioButtonClicked );
    connect( ui->opacityServerSideMinMaxRadioButton     , &QRadioButton::clicked            , this, &TransferFunctionEditorWIP::onOpacityRangeModeRadioButtonClicked );
    connect( ui->opacityUserDefinedMinDoubleSpinBox     , &QDoubleSpinBox::valueChanged     , this, &TransferFunctionEditorWIP::onOpacityUserDefinedMinChanged );
    connect( ui->opacityUserDefinedMaxDoubleSpinBox     , &QDoubleSpinBox::valueChanged     , this, &TransferFunctionEditorWIP::onOpacityUserDefinedMaxChanged );
    connect( ui->opacityMapEditPushButton               , &QPushButton::clicked             , this, &TransferFunctionEditorWIP::onOpacityMapEditorClicked );

    connect( ui->exportPushButton                       , &QPushButton::clicked             , this, &TransferFunctionEditorWIP::onExport );
    connect( ui->importPushButton                       , &QPushButton::clicked             , this, &TransferFunctionEditorWIP::onImport );
    connect( ui->applyPushButton                        , &QPushButton::clicked             , this, &TransferFunctionEditorWIP::onApply );

    updateUI();
}

void TransferFunctionEditorWIP::updateUI()
{
...
void TransferFunctionEditorWIP::clear()
{
    // Color
...
void TransferFunctionEditorWIP::disable()
{
...
void TransferFunctionEditorWIP::enable()
{
...
void TransferFunctionEditorWIP::applyTransferFunction()
{
...
void TransferFunctionEditorWIP::onNumberOfTransferFunctionValueChanged( const int numberOfTransferFunction )
{
    if( !m_transfer_function ) return;
...
// Color
void TransferFunctionEditorWIP::onColorSynthesizerChanged( const QString &colorSynthesizer )
{
    m_transfer_function->setColorSynthesizer( colorSynthesizer.toUtf8().constData() );
}

void TransferFunctionEditorWIP::onColorComboBoxChanged()
{
    updateUI();
}

void TransferFunctionEditorWIP::onColorFunctionVariableChanged( const QString &colorFunctionVariable )
{
    if( !m_transfer_function ) return;
    int colorIndex = ui->colorFunctionComboBox->currentIndex();
    if( colorIndex >= 0 && colorIndex < static_cast<int>( m_transfer_function->count() ) )
...
void TransferFunctionEditorWIP::onColorFunctionVariableEditorClicked()
{
    if( !m_transfer_function ) return;
    int colorIndex = ui->colorFunctionComboBox->currentIndex();
    if( colorIndex >= 0 && colorIndex < static_cast<int>( m_transfer_function->count() ) )
    {
        m_variable_editor_wip.setup( VariableEditorWIP::COLOR, m_transfer_function );
        m_variable_editor_wip.initialize();
        m_variable_editor_wip.adjustSize();

        if( m_variable_editor_wip.exec() == QDialog::Accepted )
        {
            QStandardItemModel* model = m_variable_editor_wip.model();
...
void TransferFunctionEditorWIP::onColorRangeModeRadioButtonClicked()
{
    int colorIndex = ui->colorFunctionComboBox->currentIndex();
    if( colorIndex >= 0 && colorIndex < static_cast<int>( m_transfer_function->count() ) )
    {
...
void TransferFunctionEditorWIP::onColorUserDefinedMinChanged( const double &colorUserDefinedMin )
{
    if( !m_transfer_function ) return;
    int colorIndex = ui->colorFunctionComboBox->currentIndex();
    if( colorIndex >= 0 && colorIndex < static_cast<int>( m_transfer_function->count() ) )
...
void TransferFunctionEditorWIP::onColorUserDefinedMaxChanged( const double &colorUserDefinedMax )
{
    if( !m_transfer_function ) return;
    int colorIndex = ui->colorFunctionComboBox->currentIndex();
    if( colorIndex >= 0 && colorIndex < static_cast<int>( m_transfer_function->count() ) )
...
void TransferFunctionEditorWIP::onColorMapEditorClicked()
{
    if( !m_transfer_function ) return;
    int colorIndex = ui->colorFunctionComboBox->currentIndex();
    if( colorIndex >= 0 && colorIndex < static_cast<int>( m_transfer_function->count() ) )
...
// Opacity
void TransferFunctionEditorWIP::onOpacitySynthesizerChanged( const QString &opacitySynthesizer )
{
    m_transfer_function->setOpacitySynthesizer( opacitySynthesizer.toUtf8().constData() );
}

void TransferFunctionEditorWIP::onOpacityComboBoxChanged()
{
    updateUI();
}

void TransferFunctionEditorWIP::onOpacityFunctionVariableChanged( const QString &opacityFunctionVariable )
{
    if( !m_transfer_function ) return;
    int opacityIndex = ui->opacityFunctionComboBox->currentIndex();
    if( opacityIndex >= 0 && opacityIndex < static_cast<int>( m_transfer_function->count() ) )
...
void TransferFunctionEditorWIP::onOpacityFunctionVariableEditorClicked()
{
    if( !m_transfer_function ) return;
    int opacityIndex = ui->opacityFunctionComboBox->currentIndex();
    if( opacityIndex >= 0 && opacityIndex < static_cast<int>( m_transfer_function->count() ) )
    {
        m_variable_editor_wip.setup( VariableEditorWIP::OPACITY, m_transfer_function );
        m_variable_editor_wip.initialize();
        m_variable_editor_wip.adjustSize();
...
void TransferFunctionEditorWIP::onOpacityRangeModeRadioButtonClicked()
{
    int opacityIndex = ui->opacityFunctionComboBox->currentIndex();
    if( opacityIndex >= 0 && opacityIndex < static_cast<int>( m_transfer_function->count() ) )
    {
...
void TransferFunctionEditorWIP::onOpacityUserDefinedMinChanged( const double &opacityUserDefinedMin )
{
    if( !m_transfer_function ) return;
    int opacityIndex = ui->opacityFunctionComboBox->currentIndex();
    if( opacityIndex >= 0 && opacityIndex < static_cast<int>( m_transfer_function->count() ) )
...
void TransferFunctionEditorWIP::onOpacityUserDefinedMaxChanged( const double &opacityUserDefinedMax )
{
    if( !m_transfer_function ) return;
    int opacityIndex = ui->opacityFunctionComboBox->currentIndex();
    if( opacityIndex >= 0 && opacityIndex < static_cast<int>( m_transfer_function->count() ) )
...
void TransferFunctionEditorWIP::onOpacityMapEditorClicked()
{
    if( !m_transfer_function ) return;
    int opacityIndex = ui->opacityFunctionComboBox->currentIndex();
    if( opacityIndex >= 0 && opacityIndex < static_cast<int>( m_transfer_function->count() ) )
...
void TransferFunctionEditorWIP::onExport()
{
...
void TransferFunctionEditorWIP::onImport()
{
    ...
    while( !in.atEnd() )
    {
        ...
        else if( type == "O" )
        {
            ...
            else if( field == "TABLE" )
            {
                ...
                m_transfer_function->at( index ).opacity.map = this->toStdVectorOpacities( table );
            }
        }
    }
    updateUI();
...
void TransferFunctionEditorWIP::onApply()
{
