...
private:
    ...
    std::size_t m_active_axis = 0; ///< index of active axis
...
public:
    ...
    void selectAxis( const std::size_t index ) { m_active_axis = index; }

    const kvs::Margins& margins() const { return m_margins; }
    const kvs::ColorMap& colorMap() const { return m_color_map; }
    std::size_t activeAxis() const { return m_active_axis; }
