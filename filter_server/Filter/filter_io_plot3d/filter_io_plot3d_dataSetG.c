...
int plot3d_dataSetG_openDataFile(Plot3Ddataset* data_set) {
    ...
    sprintf(filePath, "%s/%s.%s", Fparam.in_dir, sParams.coordinate_params.file_prefix, sParams.coordinate_params.file_suffix);
...
int plot3d_dataSetG_read3DMultiBlock(Plot3Ddataset* data_set) {
    ...
    filter_log(FILTER_LOG, "==========       Reading Grid File    G    ==========\n");
    filter_log(FILTER_LOG, "=====================================================\n");
    int err_state = RTC_OK;
    err_state |= plot3d_dataSetG_openDataFile(data_set);
    ASSERT_RTC_OK(err_state, "Failed to open grid file");
...
int plot3d_dataSetG_readCopyCoords(Plot3Ddataset* data_set, int block, int step) {
    filter_log(FILTER_LOG, "=====================================================\n");
    filter_log(FILTER_LOG, "==========       Read Write Coordinates    ==========\n");
    filter_log(FILTER_LOG, "=====================================================\n");
    int m;
    int err_state = RTC_OK;
    err_state |= plot3d_dataSetG_openDataFile(data_set);
...
void plot3d_dataSetG_findGridDimensions(Plot3Ddataset * data_set) {
    ...
    filter_log(FILTER_LOG, "## All blocks  =>[ min_x:%+.5lf   min_y:%+.5lf   min_z:%+.5lf] \t \n", data_set->xRange.min, data_set->yRange.min, data_set->zRange.min);
    filter_log(FILTER_LOG, "## All blocks  =>[ max_x:%+.5lf   max_y:%+.5lf   max_z:%+.5lf] \t \n", data_set->xRange.max, data_set->yRange.max, data_set->zRange.max);
}