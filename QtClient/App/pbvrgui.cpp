...
PBVRGUI::PBVRGUI(ExtCommand *command)
    : QMainWindow(Q_NULLPTR), ui(new Ui::PBVRGUI), particlePanel(this), filterinfoPanel(this), legendPanel(this), renderoptionPanel(this),
      systemstatusPanel(this), timecontrolPanel(this), transformPanel(this), animationControls(this), vc(this) {
  if (command == NULL) {
  ...
  log_manager.setScreen(kvs_renderarea);
  log_manager.setAnimationControls(&animationControls);
  log_manager.setTransferFuncEditor(&tf_editor);//?
  log_manager.setCoordinatePanel(&coordinatePanel);
  log_manager.setFilterinfoPanel(&filterinfoPanel);
  log_manager.setLegendPanel(&legendPanel);
  log_manager.setParticlePanel(&particlePanel);
  log_manager.setRenderoptionPanel(&renderoptionPanel);
  log_manager.setSystemstatusPanel(&systemstatusPanel);
  log_manager.setTimecontrolPanel(&timecontrolPanel);
  log_manager.setTransformPanel(&transformPanel);
  log_manager.setViewerControls(&vc);
  ...
  connect(ui->actionTransfer_Function_Editor, &QAction::triggered, this, &PBVRGUI::onTransfer_Function_EditorMenuAction);
  connect(ui->actionFilter_Information, &QAction::triggered, this, &PBVRGUI::onFilter_InformationMenuAction);
  connect(ui->actionConnect_To_Server, &QAction::triggered, this, &PBVRGUI::onConnect_To_ServerMenuAction);
  connect(ui->actionDisconnect_From_Server, &QAction::triggered, this, &PBVRGUI::onDisconnect_From_ServerMenuAction);
  connect(ui->actionExport, &QAction::triggered, this, &PBVRGUI::onExportMenuAction);
  connect(ui->actionAbout_PBVR_Client, &QAction::triggered, this, &PBVRGUI::onAbout_PBVR_ClientMenuAction);
  connect(ui->actionImport, &QAction::triggered, this, &PBVRGUI::onImportMenuAction);
  connect(ui->actionImport_Transfer_Functions, &QAction::triggered, this, &PBVRGUI::onImport_Transfer_FunctionsMenuAction);
  connect(ui->actionExport_Transfer_Functions, &QAction::triggered, this, &PBVRGUI::onExport_Transfer_FunctionsMenuAction);
  connect(ui->actionSystem_Status, &QAction::triggered, this, &PBVRGUI::onSystem_StatusMenuAction);
  connect(ui->actionRender_Options, &QAction::triggered, this, &PBVRGUI::onRender_OptionsMenuAction);
  connect(ui->actionVolume_Transform, &QAction::triggered, this, &PBVRGUI::onVolume_TransformMenuAction);
  connect(ui->actionTime_Controls, &QAction::triggered, this, &PBVRGUI::onTime_ControlsMenuAction);
  connect(ui->actionLegend_Options, &QAction::triggered, this, &PBVRGUI::onLegend_OptionsMenuAction);
  connect(ui->actionParticle_Panel, &QAction::triggered, this, &PBVRGUI::onParticle_PanelMenuAction);
  connect(ui->actionCoordinates, &QAction::triggered, this, &PBVRGUI::onCoordinatesMenuAction);
  connect(ui->actionPBVR_Client_Manual, &QAction::triggered, this, &PBVRGUI::onPBVR_Client_ManualMenuAction);
  connect(ui->actionRevert_to_Default_Layout, &QAction::triggered, this, &PBVRGUI::onRevert_to_Default_LayoutMenuAction);
  connect(ui->actionAnimation_Control, &QAction::triggered, this, &PBVRGUI::onAnimation_ControlsMenuAction);
  connect(ui->actionViewer_Controls, &QAction::triggered, this, &PBVRGUI::onViewer_ControlsMenuAction);
  connect(ui->actionZoomIn, &QAction::triggered, this, &PBVRGUI::on_actionZoomIn_triggered);
  connect(ui->actionZoomOut, &QAction::triggered, this, &PBVRGUI::on_actionZoomOut_triggered);
  connect(ui->actionXpos, &QAction::triggered, this, &PBVRGUI::on_actionXpos_triggered);
  connect(ui->actionXneg, &QAction::triggered, this, &PBVRGUI::on_actionXneg_triggered);
  connect(ui->actionYpos, &QAction::triggered, this, &PBVRGUI::on_actionYpos_triggered);
  connect(ui->actionYneg, &QAction::triggered, this, &PBVRGUI::on_actionYneg_triggered);
  ...
  connect(ui->actionAddFrame, &QAction::triggered, this, &PBVRGUI::on_actionAddFrame_triggered);
  connect(ui->actionRemoveFrame, &QAction::triggered, this, &PBVRGUI::on_actionRemoveFrame_triggered);
  connect(ui->actionPlayFrames, &QAction::triggered, this, &PBVRGUI::on_actionPlayFrames_triggered);
  connect(ui->actionplayLog,&QAction::triggered,
          this,&PBVRGUI::on_actionPlayLog_triggered);
  connect(ui->actionrecLog,&QAction::triggered,
          this,&PBVRGUI::on_actionRecLog_triggered);
...
void PBVRGUI::load_parameter_file(kvs::visclient::Argument &argument) {
  ...
  std::cout << " PBVRGUI::load_parameter_file command_q.m_parameter.m_min_server_time_step  " << extCommand->m_parameter.m_min_server_time_step
            << std::endl;
  std::cout << " PBVRGUI::load_parameter_file command_q.m_parameter.m_max_server_time_step   " << extCommand->m_parameter.m_max_server_time_step
            << std::endl;
  tmp_merger.setParam(extCommand->m_parameter.m_particle_merge_param, extCommand->m_parameter.m_min_server_time_step,
                      extCommand->m_parameter.m_max_server_time_step);
...
void PBVRGUI::onFilter_InformationMenuAction() { showDock(&filterinfoPanel, Qt::RightDockWidgetArea); }

void PBVRGUI::onConnect_To_ServerMenuAction() { showStatusMessage("Connecting to server.... \nNot implemented\n"); }

void PBVRGUI::onDisconnect_From_ServerMenuAction() { showStatusMessage("Disconnecting from server....\nNot implemented\n"); }
...
void PBVRGUI::onImport_Transfer_FunctionsMenuAction() {
  QString fileName =
      QFileDialog::getOpenFileName(this, tr("Import Transfer Function Settings File"), ".", tr("Transfer Function Files (*.tfe *.TFE *.tf *.TF )"));
  if (!fileName.isEmpty()) {
    tf_editor.importFile(fileName.toStdString());
  }
}
...
void PBVRGUI::onSystem_StatusMenuAction() { showDock(&systemstatusPanel, Qt::RightDockWidgetArea); }

void PBVRGUI::onRender_OptionsMenuAction() { showDock(&renderoptionPanel, Qt::RightDockWidgetArea); }

void PBVRGUI::onVolume_TransformMenuAction() { showDock(&transformPanel, Qt::RightDockWidgetArea); }

void PBVRGUI::onTime_ControlsMenuAction() { showDock(&timecontrolPanel, Qt::BottomDockWidgetArea); }

void PBVRGUI::onLegend_OptionsMenuAction() { showDock(&legendPanel, Qt::RightDockWidgetArea); }
...
void PBVRGUI::onCoordinatesMenuAction() { showDock(&coordinatePanel, Qt::RightDockWidgetArea); }
 ...
 */
void PBVRGUI::onRevert_to_Default_LayoutMenuAction() { restoreDefaultLayout(); }
...
void PBVRGUI::mousePressEvent(QMouseEvent *event) { QWidget::setFocus(); }
...
void PBVRGUI::on_actionAddFrame_triggered() { kvs_renderarea->animation_add(); }
void PBVRGUI::on_actionRemoveFrame_triggered() { kvs_renderarea->animation_del(); }
void PBVRGUI::on_actionPlayFrames_triggered() { kvs_renderarea->animation_play(); }

void PBVRGUI::on_actionPlayLog_triggered()
{
    log_manager.trace();
}

void PBVRGUI::on_actionRecLog_triggered()
{
    kvs_renderarea->recToggle();
    animationControls.recToggle();
    tf_editor.recToggle();
    coordinatePanel.recToggle();
//    filterinfoPanel.recToggle();
    legendPanel.recToggle();
    particlePanel.recToggle();
    renderoptionPanel.recToggle();
//    systemstatusPanel.recToggle();
//    timecontrolPanel.recToggle();
    transformPanel.recToggle();
    vc.recToggle();
}
