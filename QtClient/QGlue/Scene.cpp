...
{
    return kvs::oculus::jaea::WorldCoordinate( p ).
    ...
    m_scaling_ratio_accum(1.0)
{
    ...
    m_object_manager = new kvs::oculus::jaea::PBVRObjectManager();
...
//    std::cerr << "Scene::registerObject(): " << std::endl;
//    std::cerr << "   name = " << object->name() << std::endl;
//    std::cerr << "   hasMinMaxObjectCoords = " << object->hasMinMaxObjectCoords() << std::endl;
//    std::cerr << "   ObjCrds(before update crds) = " << object->minObjectCoord() << " ... " << object->maxObjectCoord() << std::endl;
//    {
//        std::cerr << "   m(before update crds) :" << std::endl;
...
//    std::cerr << "   ObjCrds(after update crds) = " << object->minObjectCoord() << " ... " << object->maxObjectCoord() << std::endl;


//    {
//        std::cerr << "   m(after update crds) :" << std::endl;
//        float tmp_m[16]; object->xform().toArray( tmp_m );
//        kvs::Mat4 m (tmp_m);
//        std::cerr << m << std::endl;
//    }
 ...
 */
/*==========================================================================*/
void Scene::updateXform( kvs::oculus::jaea::PBVRObjectManager* manager )
{
...
void Scene::threedcontrollerPressFunction (kvs::Vec3 hmdPos, kvs::Vec3 initialHmdPos, kvs::Vec3 hmdDir, kvs::Vec3 left_p, kvs::Vec3 right_p, kvs::Vec3 prev_left_p, kvs::Vec3 prev_right_p) {
    ...
    kvs::Vec3 center_diff = center_current - center_prev;
    kvs::Vec3 center_diff_xy( center_diff.x(), center_diff.y(), 0.0 );
    ...
    //kvs::Vec3 diff_translate = center_diff * translation_factor / scaling_ratio_accum;
//    kvs::Vec3 diff_translate = center_diff * translation_factor;
    kvs::Vec3 diff_translate = center_diff_xy * translation_factor;//kawamura mod 9/15
