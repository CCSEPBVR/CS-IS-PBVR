...
int ObjectManager::insert( kvs::ObjectBase* object )
{
...
#ifdef CHECK_INITIAL_XFORM
    {
        ...
        std::cerr << "ObjectManager::insert(): object=" << object->name() << std::endl;
    ...
    this->push_centering_xform();
    {
        ...
        if (ObjectNameUtil::isToIgnoreTranslationFactor(object->name()) == false)
...
void ObjectManager::change( int id, ObjectBase* object, bool delete_flag )
{
...
#ifdef CHECK_INITIAL_XFORM
    {
        ...
        std::cerr << "ObjectManager::change(): id=" << id << ", object=" << object->name() << std::endl;
...
void ObjectManager::change( std::string name, kvs::ObjectBase* object, bool delete_flag )
{
...
#ifdef CHECK_INITIAL_XFORM
    {
        ...
        std::cerr << "ObjectManager::change(): name=" << name << ", object=" << object->name() << std::endl;
        std::cerr << "before insert ------------------------" << std::endl;
        i = -1;
        obj_name = "ObjectManager";
        e[0] = this->minExternalCoord();
...
{
    std::cerr << "#----------------------#      ObjectManager::resetXform()" << std::endl;
 ...
 */
/*==========================================================================*/
void ObjectManager::scaleObject(kvs::ObjectBase* target_obj, const kvs::Vec3& scaling )
{
    kvs::ObjectBase* object = this->get_control_target();
    ...
    while ( registered_object != last )
    {
        if ((*registered_object) == target_obj) {
            std::cout << "#### " << (*registered_object)->name() << std::endl;
            (*registered_object)->multiplyXform( x );
            return;
        }
        ++registered_object;
