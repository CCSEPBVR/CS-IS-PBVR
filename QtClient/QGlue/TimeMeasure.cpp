...
std::string TimeMeasure::At(std::string file, int line) { return std::string("at ").append(file).append(":").append(std::to_string(line)); }

bool TimeMeasure::isLastIndex() { return (m_index == (m_max_index - 1)); }
...
void TimeMeasure::record(std::string key, long long int elapsed) { record(key, static_cast<double>(elapsed)); }
