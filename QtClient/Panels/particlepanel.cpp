...
//    m_keep_final_value[item_num]->setText(buf);
    ...
    {
        LogManager lg;
        lg.Write("particlepanel.cpp",__func__,"index", std::to_string(item_num));
    }
}
...
void ParticlePanel::onItemToAddIndexChange(int index)
{
    ...
    if( m_isRec )
    {
        ...
        {
            LogManager lg;
            lg.Write("particlepanel.cpp",__func__,"index", std::to_string(index));
        }
    }
}
...
{
    QString  particle_file = QFileDialog::getOpenFileName(this, tr("Particle File"), ".", tr("Particle Files (*.stl *.kvsml)"));
    if(!particle_file.isEmpty()&& !particle_file.isNull()){
        ...
        if(particle_file.contains(".stl"))
        {
            particle_file.chop(10);
        }
        if(particle_file.contains(".kvsml"))
        {
            particle_file.chop(28);
        }
    ...
    {
        LogManager lg;
        lg.Write("particlepanel.cpp",__func__,"filePath", particle_file.toStdString());
    }
}
...
void ParticlePanel::onCloseButtonClick()
{
    ...
    {
        LogManager lg;
        lg.Write("particlepanel.cpp",__func__,"closeButton", "clicked");
    }
}
...
void ParticlePanel::onDivColorChanged()
{
    ...
    if(ui->itemToAdd->currentIndex() >= 5 && ui->itemToAdd->currentIndex() <= 9  ){
        ...
        {
            LogManager lg;
            lg.Write("particlepanel.cpp",__func__,"kvs RGB Color", std::to_string(m_param->m_particles[ui->itemToAdd->currentIndex()+1].m_polygon_color.r()) + "," + std::to_string(m_param->m_particles[ui->itemToAdd->currentIndex()+1].m_polygon_color.g()) + "," + std::to_string(m_param->m_particles[ui->itemToAdd->currentIndex()+1].m_polygon_color.b()));
        }
    }
}
...
void ParticlePanel::changeOpacityValue()
{
    ...
    if(ui->itemToAdd->currentIndex() >= 5 && ui->itemToAdd->currentIndex() <= 9  ){
        ...
        {
            LogManager lg;
            lg.Write("particlepanel.cpp",__func__,"opacity", std::to_string(m_param->m_particles[ui->itemToAdd->currentIndex()+1].m_polygon_opacity));
        }
    }
}

void ParticlePanel::logPlay(std::string funcName, std::string str[])
{
    if(funcName.compare("onDeleteButtonClick") == 0)
    {
        this->onDeleteButtonClick();
    }

    if(funcName.compare("onItemToAddIndexChange") == 0)
    {
        ui->itemToAdd->setCurrentIndex(std::stoi(str[0]));
    }

    if(funcName.compare("onBrowseButtonClick") == 0)
    {
        ui->particleFile->setText(QString::fromStdString(str[0]));
    }

    if(funcName.compare("onSetButtonClick") == 0)
    {
        this->onSetButtonClick();
    }

    if(funcName.compare("onCloseButtonClick") == 0)
    {
    ...
    {
        m_param->m_particles[ui->itemToAdd->currentIndex()+1].m_polygon_color = kvs::RGBColor(std::stoi(str[0]),std::stoi(str[1]),std::stoi(str[2]));
    }

    if(funcName.compare("changeOpacityValue") == 0)
    {
        m_param->m_particles[ui->itemToAdd->currentIndex()+1].m_polygon_opacity = std::stod(str[0]);
        ui->doubleSpinBox->setValue(std::stod(str[0]));
    }
}
