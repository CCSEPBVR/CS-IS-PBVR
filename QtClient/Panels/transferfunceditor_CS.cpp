...
void TransferFuncEditor::apply()
{
   std::cout<<"---\n m_color_transfer_function_synthesis:"<< m_doc.m_color_transfer_function_synthesis.c_str()<<std::endl;
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"apply", "clicked");
    }
}
...
//        this->apply();
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"lock", std::to_string(locked));
    }
}
...
//    sprintf(tag_o, "%s_var_o", tmp_omf.c_str());
    ...
    std::cout << "panel, " <<extCommand->m_result.m_var_range.min( tag_c ) << std::endl;
    std::cout << "panel, " <<extCommand->m_result.m_var_range.max( tag_c ) << std::endl;
    std::cout << "panel, " <<extCommand->m_result.m_var_range.min( tag_o ) << std::endl;
    std::cout << "panel, " <<extCommand->m_result.m_var_range.max( tag_o ) << std::endl;
...
//    cFuncEditor.show();
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"colorFunctionButton", "clicked");
    }
}
...
void TransferFuncEditor::onColorButtonClicked()
{
    ...
    if (choice==QDialog::Accepted){
        ...
        if( m_isRec )
        {
            kvs::ValueArray<int> colormap;
            colormap.allocate(cmap.resolution()*3);
            for(int i = 0; i < cmap.resolution() * 3; i++ )
            {
                colormap.at(i) = (int)cmap.table().at(i);
            }

            LogManager lg;
            lg.Write("transferfunceditor_CS.cpp",__func__,"colormap", colormap.format("",","));
        }
    }
...
void TransferFuncEditor::onColorFunctionSynthEdited(const QString &arg1)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"colorFunctionSynth", m_doc.m_color_transfer_function_synthesis);
    }
}
...
void TransferFuncEditor::onColorMapFunctionChanged(int index)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"colormapIndex", std::to_string(index));
    }
}
...
void TransferFuncEditor::onNumTransferChanged(int val)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"numberOfTransfer", std::to_string(val));
    }
    return;
}
...
//    oFuncEditor.show();
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"opacityFunctionButton", "clicked");
    }
}
...
void TransferFuncEditor::onOpacityFunctionSynthEdited(const QString &arg1)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"opacityFunctionSynth", m_doc.m_opacity_transfer_function_synthesis);
    }
}
...
void TransferFuncEditor::onOpacityButtonClicked()
{
    ...
    if (choice==QDialog::Accepted){
        ...
        {
            LogManager lg;
            lg.Write("transferfunceditor_CS.cpp",__func__,"opacitymap", omap.table().format("",","));
        }
    }
...
void TransferFuncEditor::onResetButtonClicked()
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"resetButton", "clicked");
    }
}
...
void TransferFuncEditor::onTransferFunctionVarColorEdited(const QString &arg1)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"transferFunctionVariableColor", fName + "," + variable);
    }
}
...
void TransferFuncEditor::onTransferFunctionVarOpacityEdited(const QString &arg1)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"transferFunctionVariableOpacity", fName + "," + variable);
    }
}
...
void TransferFuncEditor::onTFminColorChanged(double arg1)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"colormapFunction,TFColorRange", ui->colormapFunction->currentText().toStdString() + "," +
                 std::to_string(ui->transfer_function_min_color->value()) + "," + std::to_string(ui->transfer_function_max_color->value()));
    }
}
...
void TransferFuncEditor::onTFminOpacityChanged(double arg1)
{
    ...
    {
        LogManager lg;
        lg.Write("transferfunceditor_CS.cpp",__func__,"opacitymapFunction,TFColorRange", ui->opacitymapFunction->currentText().toStdString() + "," +
                 std::to_string(ui->transfer_function_min_opacity->value()) + "," + std::to_string(ui->transfer_function_max_opacity->value()));
    }
}
...
void TransferFuncEditor::logPlay(std::string funcName, std::string str[])
{
    if(funcName.compare("apply") == 0 )
    {
        this->apply();
    }

    if(funcName.compare("onLockOpacityRange") == 0 )
    {
        this->onLockOpacityRange(std::stoi(str[0]));
    }

    if(funcName.compare("onLockColorRange") == 0 )
    {
        this->onLockColorRange(std::stoi(str[0]));
    }

////    if(funcName.compare("onCfuncClicked") == 0 )
////    {

////    }

    if(funcName.compare("onColorButtonClicked") == 0)
    {
        kvs::ColorMap cmap( 256, 0.0, 1.0);
        const float stride = (1.0 - 0.0) / (256 - 1);
        float x = 0.0;

        for(size_t i = 0; i < 256 * 3; i += 3, x += stride)
        {
            cmap.addPoint( x,kvs::RGBColor(std::stoi(str[i]),std::stoi(str[i + 1]), std::stoi(str[i + 2])));
        }
        cmap.create();
        this->colorEditor.m_color_map_palette->setColorMap( cmap );
        this->colorEditor.m_color_map_palette->m_color_map_initial = cmap;
        m_color_map_palette->setColorMap( cmap );
        int n_select_color = ui->colormapFunction->currentIndex()+1;
        NamedTransferFunctionParameter *to = this->m_doc.getColorTransferFunction(n_select_color);
        to->setColorMap(cmap);
    }

    if(funcName.compare("onColorFunctionSynthEdited") == 0 )
    {
        ui->color_function_synth->setText( QString::fromStdString( str[0] ) );
        this->onColorFunctionSynthEdited( QString::fromStdString( str[0]) );
    }

    if(funcName.compare("onColorMapFunctionChanged") == 0 )
    {
        ui->colormapFunction->setCurrentIndex( std::stoi( str[0] ));
        this->onColorMapFunctionChanged( std::stoi( str[0] ));
    }

    if(funcName.compare("onNumTransferChanged") == 0 )
    {
        ui->num_transfer->setValue(std::stoi(str[0]));
//        this->onNumTransferChanged(std::stoi(str[0]));
    }

////    if(funcName.compare("onOfuncClicked") == 0 )
////    {

////    }

    if(funcName.compare("onOpacityFunctionSynthEdited") == 0 )
    {
        ui->opacity_function_synth->setText( QString::fromStdString( str[0] ));
        this->onOpacityFunctionSynthEdited( QString::fromStdString( str[0] ));
    }

    if(funcName.compare("onOpacityButtonClicked") == 0 )
    {
        kvs::OpacityMap omap( 256, 0.0,1.0);
        const float stride = (1.0 - 0.0) / (256 - 1);
        float x = 0.0;
        for(size_t i = 0; i < 256; ++i, x += stride)
        {
         omap.addPoint( x, std::stod(str[i]) );
        }
        omap.create();
        this->opacityEditor.m_opacity_map_palette->setOpacityMap( omap );
        this->opacityEditor.m_opacity_map_palette->m_opacity_map_initial = omap;
        m_opacity_map_palette->setOpacityMap( omap );
        int n_select_opacity = ui->opacitymapFunction->currentIndex()+1;
        NamedTransferFunctionParameter *to = this->m_doc.getOpacityTransferFunction(n_select_opacity);
        to->setOpacityMap(omap);
    ...
    if(funcName.compare("onOpacityMapFunctionChanged") == 0 )
    {
        ui->opacitymapFunction->setCurrentIndex( std::stoi( str[0] ));
        this->onOpacityMapFunctionChanged( std::stoi( str[0] ));
    }

//    if(funcName.compare("onResetButtonClicked") == 0 )
//    {
//        this->onResetButtonClicked();
//    }

    if(funcName.compare("onTransferFunctionVarColorEdited") == 0 )
    {
        ui->transfer_function_var_color->setText( QString::fromStdString( str[1] ));
        this->onTransferFunctionVarColorEdited( QString::fromStdString( str[1] ));
    }

    if(funcName.compare("onTransferFunctionVarOpacityEdited") == 0 )
    {
        ui->transfer_function_var_opacity->setText( QString::fromStdString( str[1] ));
        this->onTransferFunctionVarOpacityEdited( QString::fromStdString( str[1] ));
    }

    if(funcName.compare("onTFminColorChanged") == 0 )
    {
        ui->transfer_function_min_color->setValue( std::stod( str[2] ) );
    }

    if(funcName.compare("onTFmaxColorChanged") == 0 )
    {
        ui->transfer_function_max_color->setValue( std::stod( str[3]) );
    }

    if(funcName.compare("onTFminOpacityChanged") == 0 )
    {
        ui->transfer_function_min_opacity->setValue( std::stod( str[2]) );
    }

    if(funcName.compare("onTFmaxOpacityChanged") == 0)
    {
        ui->transfer_function_max_opacity->setValue( std::stod( str[3]) );
    }
}
