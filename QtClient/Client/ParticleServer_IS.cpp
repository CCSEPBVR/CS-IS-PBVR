...
//      message.transfunc = const_cast<TransferFunction*>(&param.transferFunction);
        ...
        //4 debug
//        message.m_arrow_flag = false;
//        // add by shimomura  20240116
//        std::ifstream ifs;
//        ifs.open( "/Users/shimomurakazuya/SGI/UNPBVR/IntegPBVRv220/test.txt", std::ios::in );
//        if (ifs.is_open() == true)
//        {
//            for(int i = 0; i<3; i++) //3 components of arrow coord.
//            {
//                ifs >> message.m_arrow_component[i];
//                std::cout << "message.m_arrow_component["<< i << "] = " << message.m_arrow_component[i] <<std::endl;
//            }
//            message.m_arrow_flag = true;
//        }
//        else
//        {
//            std::cout << " test.txt, arrrow file can't open !!!!!"  <<std::endl;
//        }
//        ifs.close();

//        for(int i = 0; i<16; i++) //3 components of arrow coord.
//        {
//            message.m_glyph_xform = param->m_Glyph_Xform;
//            message.m_obj_xform = param->m_ObjectManager_Xform;
////            std::cout << "message.m_arrow_component["<< i << "] = " << message.m_arrow_component[i] <<std::endl;
//        }

        message.m_arrow_flag = true;  // force to send glyph
        param.m_Glyph_Xform.toArray(message.m_glyph_xform );
        param.m_ObjectManager_Xform.toArray(message.m_obj_xform);

//        std::cout << __func__ << __LINE__ << std::endl;
//        std::cout << param.m_Glyph_Xform.toMatrix()<< std::endl;
//        std::cout << param.m_ObjectManager_Xform.toMatrix()<< std::endl;


//                for(int i = 0; i<16; i++) //3 components of arrow coord.
//                {
//                    std::cout << "message.m_Glyph_Xform["<< i << "] = " << message.m_glyph_xform[i] <<std::endl;
//                }
//                for(int i = 0; i<16; i++) //3 components of arrow coord.
//                {
//                    std::cout << "message.m_obj_xform["<< i << "] = " << message.m_obj_xform[i] <<std::endl;
//                }

        // for (int i=0; i<3; i++)
        // {
        //     message.m_arrow_component[i] = i ;
        // }

         for (int i=0; i<3; i++)
         {
             message.m_arrow_component[i] = param.m_Glyph_coord[i];
         }

//         message.m_arrow_component = param.m_Glyph_coord;
...
//      client.recvMessage( reply );
    ...
    result->flag_send_bins=reply.m_flag_send_bins;
    result->tf_count=reply.m_transfer_function_count;
    result->arrow_flag =false;
    if(reply.m_arrow_flag == true)
    {
    result->arrow_component[0] = reply.m_arrow_component[0];
    result->arrow_component[1] = reply.m_arrow_component[1];
    result->arrow_component[2] = reply.m_arrow_component[2];
    result->arrow_flag = reply.m_arrow_flag;
    }
