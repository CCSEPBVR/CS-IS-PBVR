    ...
    bool is_client_server_mode)
{
    ...
    for ( size_t n = 0; n < num; n++ )
    {
        ...
        if ( m_parameter.m_particles[n].m_file_path == "server" )
        {
            //std::cout << "========== obj : " << obj << std::endl;
            //std::cout << "========== server : " << &server_particle << std::endl;
                if(step >= m_initial_step[n] && step <= m_final_step[n])
                {
                ...
                else if(step <= m_initial_step[n] && m_parameter.m_particles[n].m_keep_initial_step)
                {
                    if (server_particle != NULL)
                    {
                        ( *obj ) +=  (*server_particle);
                    }
                }else if(step >= m_final_step[n] && m_parameter.m_particles[n].m_keep_final_step)
                {
                    if (server_particle != NULL)
                    {
                        ( *obj ) +=  (*server_particle);
