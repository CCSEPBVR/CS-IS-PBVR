...
//                sp_level= m_command->m_parameter.m_detailed_subpixel_level;
            ...
            if(     m_front_object->numberOfVertices() != 0 &&
                ...
                std::cerr << "############# set before_Xform" << std::endl;
            }
            {
                std::cerr << "m_screen->scene()->objectManager()->externalCenter() = " << std::endl;
                std::cerr << m_screen->scene()->objectManager()->externalCenter() << std::endl;
                std::cerr << "m_screen->scene()->objectManager()->objectCenter() = " << std::endl;
                std::cerr << m_screen->scene()->objectManager()->objectCenter() << std::endl;
                std::cerr << "m_screen->scene()->objectManager()->normalize() = " << std::endl;
                std::cerr << m_screen->scene()->objectManager()->normalize() << std::endl;
            }
            {
                float xform_array[16];
                m_screen->scene()->objectManager()->get_centering_xform( m_screen->scene()->objectManager() ).toArray(xform_array);
                std::cerr << "m_screen->scene()->objectManager()->get_centering_xform() = " << std::endl;
                std::cerr << " | " << xform_array[0*4+0] << "  " << xform_array[0*4+1] << "  " << xform_array[0*4+2] << "  " << xform_array[0*4+3] << std::endl;
                std::cerr << " | " << xform_array[1*4+0] << "  " << xform_array[1*4+1] << "  " << xform_array[1*4+2] << "  " << xform_array[1*4+3] << std::endl;
                std::cerr << " | " << xform_array[2*4+0] << "  " << xform_array[2*4+1] << "  " << xform_array[2*4+2] << "  " << xform_array[2*4+3] << std::endl;
                std::cerr << " | " << xform_array[3*4+0] << "  " << xform_array[3*4+1] << "  " << xform_array[3*4+2] << "  " << xform_array[3*4+3] << std::endl;
            }
            {
                float xform_array[16];
                before_Xform.toArray(xform_array);
                std::cerr << "before_Xform = " << std::endl;
                std::cerr << " | " << xform_array[0*4+0] << "  " << xform_array[0*4+1] << "  " << xform_array[0*4+2] << "  " << xform_array[0*4+3] << std::endl;
                std::cerr << " | " << xform_array[1*4+0] << "  " << xform_array[1*4+1] << "  " << xform_array[1*4+2] << "  " << xform_array[1*4+3] << std::endl;
                std::cerr << " | " << xform_array[2*4+0] << "  " << xform_array[2*4+1] << "  " << xform_array[2*4+2] << "  " << xform_array[2*4+3] << std::endl;
                std::cerr << " | " << xform_array[3*4+0] << "  " << xform_array[3*4+1] << "  " << xform_array[3*4+2] << "  " << xform_array[3*4+3] << std::endl;
...
#ifdef ENABLE_LIBLAS
            if(m_screen->scene()->hasObject("LAS_MODEL") == true)
            {
            m_front_object->setMinMaxObjectCoords(m_screen->scene()->objectManager()->object("LAS_MODEL")->minObjectCoord(),m_screen->scene()->objectManager()->object("LAS_MODEL")->maxObjectCoord());
            m_front_object->setMinMaxExternalCoords(m_screen->scene()->objectManager()->object("LAS_MODEL")->minObjectCoord(),m_screen->scene()->objectManager()->object("LAS_MODEL")->maxObjectCoord());
            }
#endif
            m_screen->attachPointObject(m_front_object,sp_level);
            {
                std::cerr << "m_front_object->minObjectCoord() = " << std::endl;
                std::cerr << " | " << m_front_object->minObjectCoord() << std::endl;
                std::cerr << "m_front_object->maxObjectCoord() = " << std::endl;
                std::cerr << " | " << m_front_object->maxObjectCoord() << std::endl;

#ifdef VR_MODE
                kvs::ObjectBase* boundlingbox = m_screen->scene()->objectManager()->object(OBJ_NAME_BOUNDING_BOX);
                if(boundlingbox != nullptr){
                    std::cerr << "boundlingbox->minObjectCoord() = " << std::endl;
                    std::cerr << " | " << boundlingbox->minObjectCoord() << std::endl;
                    std::cerr << "boundlingbox->maxObjectCoord() = " << std::endl;
                    std::cerr << " | " << boundlingbox->maxObjectCoord() << std::endl;
                } else{
                    std::cerr << "boundlingbox is null" << std::endl;
                }
#endif
            }
            ...
            {
                float xform_prev[16];
                m_screen->scene()->objectManager()->xform().toArray(xform_prev);
                std::cerr << "TimerEvent::update(): after call attachPointObject()" << std::endl;
...
//                        m_front_object->setXform( Xform_new );
                        ...
                        //shimomura20240128
                        //float glph_coord[3] = m_screen -> touch_controller() -> glyphcoord();
