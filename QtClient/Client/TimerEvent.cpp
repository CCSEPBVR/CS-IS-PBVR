...
void TimerEvent::update( kvs::TimeEvent* event )
{
    ...
    else if ( comstatus == ComThread::Exit )
    {
        ...
        if ( m_command->m_is_under_animation && m_command->m_particle_assign_flag )
        {
            ...
            {
                //std::cout << __LINE__ << ":" << m_command->m_parameter.m_time_step << std::endl;
//                *m_front_object = *( m_command->m_detailed_particles[m_command->m_parameter.m_time_step] );
                //全タイムステップの配列確保廃止に伴う変更 in CS 1.17.3
 //                *m_front_object = *( m_command->m_detailed_particles[m_command->m_parameter.m_time_step] );
                *m_front_object = *( m_command->m_detailed_particles[m_command->m_detailed_particles.size()-1] );

                // 20221221 yodo modified:
                // old code using m_detailed_subpixel_level is CS 1.14 based.
                // new code using m_detailed_particles->size() is CS 1.17 based.
           ...
           kvs::PointObject* m_front_object = new PointObject();
            ...
            std::cout << "m_front_object : nparticles=" << m_front_object->numberOfVertices() / 3 << std::endl;

            m_front_object->setMinMaxObjectCoords(kvs::Vector3f(extCommand->PVBRmincoords),kvs::Vector3f(extCommand->PVBRmaxcoords));
#endif // 0
#ifdef ENABLE_BUGSHOOTING_MODE
            //bug調査
            // JAEA
            //m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875,-437,1037*20-50000),kvs::Vector3f(71054.9,60400.8,3346*20-50000));
            // 20210613a insight
            //m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875,-437,1037*20-63000),kvs::Vector3f(71054.9,60400.8,3346*20-63000));
            // 20210613b insight
            //m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875,-437,1037*20-70000),kvs::Vector3f(71054.9,60400.8,3346*20-70000));
            // 20210613c insight
            //m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875,-437,1037*20-50000),kvs::Vector3f(0,60400.8,3346*20-50000));
            // 20210614 nakamura
            float coeff = 1.0;
            //m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875*coeff, -437*coeff, 1037*coeff),kvs::Vector3f(71054.9*coeff, 60400.8*coeff, 3346*coeff));
            m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875*coeff, -437*coeff, 1037*coeff + 3000),kvs::Vector3f(71054.9*coeff, 60400.8*coeff, 3346*coeff + 3000));
            //m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875*coeff, -440*coeff, 0*coeff),kvs::Vector3f(71054.9*coeff, -437*coeff, 10000*coeff));
            //m_front_object->setMinMaxExternalCoords(kvs::Vector3f(-138.875*coeff, 1000*coeff, 0*coeff),kvs::Vector3f(71054.9*coeff, 1100*coeff, 10000*coeff));
#else // ENABLE_BUGSHOOTING_MODE

#ifndef ENABLE_EXTCRD_SCALING_FOR_DEMO
            // 20220703 yodo@meshman.jp append for debug
            float *min_crd = extCommand->PBVRmincoords;
            float *max_crd = extCommand->PBVRmaxcoords;
            fprintf (stderr, "DEBUG: TimerEvnet::update(): \n");
            ...
            kvs::Vec3 extMin = m_front_object->minExternalCoord();
            kvs::Vec3 extMax = m_front_object->maxExternalCoord();
            fprintf (stderr, "DEBUG:   PBVRmincoords: (% 16f, % 16f, % 16f\n", extMin.x(), extMin.y(), extMin.z());
            fprintf (stderr, "DEBUG:   PBVRmaxcoords: (% 16f, % 16f, % 16f\n", extMax.x(), extMax.y(), extMax.z());

#ifdef ENABLE_SET_PBVRCOORDS_TO_FRONT_OBJECT
            m_front_object->setMinMaxExternalCoords(kvs::Vector3f(extCommand->PBVRmincoords),kvs::Vector3f(extCommand->PBVRmaxcoords));
#endif // ENABLE_SET_PBVRCOORDS_TO_FRONT_OBJECT


#else //ENABLE_EXTCRD_SCALING_FOR_DEMO
            // 20220703 yodo@meshman.jp modified.
            float min_crd[] = {extCommand->PVBRmincoords[0],extCommand->PVBRmincoords[1],extCommand->PVBRmincoords[2]};
            float max_crd[] = {extCommand->PVBRmaxcoords[0],extCommand->PVBRmaxcoords[1],extCommand->PVBRmaxcoords[2]};
            float center[] = {(min_crd[0] + max_crd[0])/2.0f, (min_crd[1] + max_crd[1])/2.0f, (min_crd[2] + max_crd[2])/2.0f};
            float len_crd[3] = {max_crd[0]-min_crd[0], max_crd[1]-min_crd[1], max_crd[2]-min_crd[2]};
            const float *ref_len = (extCommand->referenceMaxCoord - extCommand->referenceMinCoord).data();
            const float *ref_center = ((extCommand->referenceMaxCoord + extCommand->referenceMinCoord) / 2.0f).data();
            const float delta_center[] = {ref_center[0] - center[0], ref_center[1] - center[1], ref_center[2] - center[2]};
            if (std::abs(len_crd[0]) > 1e-100 && std::abs(len_crd[1]) > 1e-100 && std::abs(len_crd[2]) > 1e-100) {
                float len_rat[3] = {ref_len[0] / len_crd[0], ref_len[1] / len_crd[1], ref_len[2] / len_crd[2]};
                float zoom_ratio = 1.0f;
                zoom_ratio = (len_rat[0] > zoom_ratio) ? len_rat[0] : zoom_ratio;
                zoom_ratio = (len_rat[1] > zoom_ratio) ? len_rat[1] : zoom_ratio;
                zoom_ratio = (len_rat[2] > zoom_ratio) ? len_rat[2] : zoom_ratio;

                float mid_crd[] = {(max_crd[0] + min_crd[0])/2.0f, (max_crd[1] + min_crd[1])/2.0f, (max_crd[2] + min_crd[2])/2.0f};
                min_crd[0] = mid_crd[0] - len_crd[0] * zoom_ratio / 2.0f;
                min_crd[1] = mid_crd[1] - len_crd[1] * zoom_ratio / 2.0f;
                min_crd[2] = mid_crd[2] - len_crd[2] * zoom_ratio / 2.0f;
                max_crd[0] = mid_crd[0] + len_crd[0] * zoom_ratio / 2.0f;
                max_crd[1] = mid_crd[1] + len_crd[1] * zoom_ratio / 2.0f;
                max_crd[2] = mid_crd[2] + len_crd[2] * zoom_ratio / 2.0f;
            }

            min_crd[0] += delta_center[0];
            min_crd[1] += delta_center[1];
            min_crd[2] += delta_center[2];
            max_crd[0] += delta_center[0];
            max_crd[1] += delta_center[1];
            max_crd[2] += delta_center[2];

            fprintf (stderr, "DEBUG: TimerEvnet::update(): \n");
            fprintf (stderr, "DEBUG:   PBVRmincoords: (% 16f, % 16f, % 16f\n", min_crd[0], min_crd[1], min_crd[2]);
            fprintf (stderr, "DEBUG:   PBVRmaxcoords: (% 16f, % 16f, % 16f\n", max_crd[0], max_crd[1], max_crd[2]);

            m_front_object->setMinMaxExternalCoords(kvs::Vector3f(min_crd),kvs::Vector3f(max_crd));
#endif // ENABLE_EXTCRD_SCALING_FOR_DEMO

#endif // ENABLE_BUGSHOOTING_MODE


            // 20230110 : copy from cs_pbvr 1.17.3
...
//            {
//                std::cerr << "TimerEvent::update() : after call RenderArea::attachPointObject(): " << std::endl;
//                float tmp_mm[16]; m_screen->scene()->objectManager()->xform().toArray( tmp_mm );
//                kvs::Mat4 mm (tmp_mm);
//                std::cerr << "m(m)=" << std::endl;
//                std::cerr << mm << std::endl;
//                for (int iobj = 0; iobj < m_screen->scene()->objectManager()->numberOfObjects(); iobj++) {
//                    float tmp_m[16]; m_screen->scene()->objectManager()->xform(iobj).toArray( tmp_m );
//                    kvs::Mat4 m (tmp_m);
//                    std::cerr << "m(" << iobj << ")=" << std::endl;
//                    std::cerr << m << std::endl;
//                }
//            }
...
//            {
//                std::cerr << "TimerEvent::update() : after call translate/scale/rotate of ObjectManager: " << std::endl;
//                float tmp_mm[16]; m_screen->scene()->objectManager()->xform().toArray( tmp_mm );
//                kvs::Mat4 mm (tmp_mm);
//                std::cerr << "m(m)=" << std::endl;
//                std::cerr << mm << std::endl;
