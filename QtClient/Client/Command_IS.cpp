...
{
    extCommand->registerGlyph( param->m_port );
    // Viewer のスケール正規化用.
    ...
    param->m_Glyph_coord = extCommand->m_screen->scene()->m_current_glyph_pos;
    param->m_Glyph_Xform = extCommand->m_screen->GlyphXform();
    param->m_ObjectManager_Xform = extCommand->m_screen-> ObjectManagerXform();
...
//        if( param->m_port != 60000)
        if( m_parameter.m_host_flag == false )
        {
//            extCommand->updateGuestGlyph();
//            std::cout << "[GLYPH]:" << __LINE__ << std::endl;
            std::cout << result->arrow_component[0] << "," << result->arrow_component[1] << "," << result->arrow_component[2] << std::endl;
            extCommand->m_screen->scene()->m_current_glyph_pos = kvs::Vec3(result->arrow_component[0],result->arrow_component[1],result->arrow_component[2]);
        }
...
//delete show (jupiter)
    ...
    // change view
    std::cout << __LINE__ << std::endl;
    std::cout << PBVRmincoords[0] << std::endl;
    std::cout << PBVRmincoords[1] << std::endl;
    std::cout << PBVRmincoords[2] << std::endl;
    std::cout << PBVRmaxcoords[0] << std::endl;
    std::cout << PBVRmaxcoords[1] << std::endl;
    std::cout << PBVRmaxcoords[2] << std::endl;
