...
#ifdef ENABLE_LIBLAS
    std::string m_las_model;
    bool m_las_model_flag;
#endif
    //OSAKI
...
// APPEND END   BY)M.Tanaka 2015.03.11
        ...
        addOption("host", "host setting", 0, false);
...
#ifdef ENABLE_LIBLAS
        addOption("las_model", "background LAS model file", 1, false);
#endif
        //OSAKI

#ifdef DISP_MODE_VR
        addOption("lefty", "lefty mode for virtual mouse (x=click, right stick=move)", 0, false);
#endif // DISP_MODE_VR
...
//      m_header = this->value<std::string>();
...
#ifdef ENABLE_LIBLAS
        if(hasOption("las_model"))
        {
            m_las_model = this->optionValue<std::string>("las_model");
            m_las_model_flag = true;
        }
#endif
        //OSAKI
        ...
        if (hasOption("lefty")) {
            ...
            std::cout << "Lefty mode on -> virtual mouse move = right stick, click=X button." << std::endl;
