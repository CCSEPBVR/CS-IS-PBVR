...
bool cvt::UnstructuredPfi::write( const char* const filename )
{
    if ( auto f = fopen( filename, "w" ) )
    {
        ...
        std::string type_of_elements_str  = "CELLTYPE=" + std::to_string(type_of_elements) + "\n";
        ...
        for ( int i = 0; i < max_sub_volume_id; ++i )
        {
        std::string element_counts_str = "ELEMENT_COUNT_NUM_" + std::to_string(i) +  "=" + std::to_string(element_counts[0][i]) + "\n";
