 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_openStepFile(Plot3Ddataset* data_set, int step) {
    ...
    if (param->end_step + param->start_step > 0) {
        sprintf(filePath, "%s/%s%06lld.%s", param->in_dir, data_set->params->solution_params.file_prefix, step + param->start_step, data_set->params->solution_params.file_suffix);
    } else {
        sprintf(filePath, "%s/%s.%s", param->in_dir, data_set->params->solution_params.file_prefix, data_set->params->solution_params.file_suffix);
    }
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_openStepFiles(Plot3Ddataset* data_set) {
    int step;
    int status = RTC_OK;
 ...
 * @return
 */
void FilterIoPlot3d::plot3d_dataSetS_closeStepFile(Plot3Ddataset* data_set, int step) {
    timeStep* tStep = &(data_set->timeSteps[step]);
    GridFile* gridFile = &(tStep->solution_file);
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_closeStepFiles(Plot3Ddataset* data_set) {
    int step;
    for (step = 0; step < data_set->nSteps; step++) {
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_allocateBlockTypes(Plot3Ddataset* data_set, int nGrids) {
    // ref_malloc(data_set->solution_blocks, (sizeof (Plot3DSolutionBlock) * nGrids));
    data_set->solution_blocks = (Plot3DSolutionBlock*)malloc(sizeof (Plot3DSolutionBlock) * nGrids);
    // add by @hira at 2016/12/01
    memset(data_set->solution_blocks, 0x00, sizeof (Plot3DSolutionBlock) * nGrids);
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_readBlockCounts(Plot3Ddataset* data_set) {
 ...
 */
void FilterIoPlot3d::plot3d_dataSetS_dumpBlocks(Plot3Ddataset *data_set, const char* filePath) {
    int n;
    printf("\n==========    Dumping Grid File       ==========\n");
 ...
 */
int FilterIoPlot3d::plot3d_dataSetS_indexBlocks(Plot3Ddataset* data_set) {
    int m;
    int dim[3];
    ...
    data_set->solution_offsets = (Int64*)malloc(sizeof (Int64)* data_set->nBlocks);
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_readBlockData(Plot3Ddataset* data_set, int step) {
    int m;
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_deAllocateMemory(Plot3Ddataset* data_set) {
...
int FilterIoPlot3d::plot3d_dataSetS_skipToBlock(Plot3Ddataset* data_set, int block, int step) {
    if (data_set == NULL) return RTC_NG;
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_read3DSingleBlock(Plot3Ddataset* data_set, int block, int step) {
    if (this->m_filter_info == NULL) return RTC_NG;
    if (this->m_filter_info->m_param == NULL) return RTC_NG;
    FilterParam *param = this->m_filter_info->m_param;
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_read3DMultiBlock(Plot3Ddataset* data_set, int step) {
    LOGOUT(FILTER_LOG, (char*)"=====================================================\n");
    LOGOUT(FILTER_LOG, (char*)"==========       Reading Grid File    S    ==========\n");
    LOGOUT(FILTER_LOG, (char*)"=====================================================\n");
    int err_state = RTC_OK;
 ...
 * @return
 */
int FilterIoPlot3d::plot3d_dataSetS_copyToArray(Plot3Ddataset* data_set, int step, int offset, int skip) {
    int m;
    Plot3DSolutionBlock* sGrid;
...
int FilterIoPlot3d::plot3d_dataSetS_readCopySolution(Plot3Ddataset* data_set, int step, int block) {
    if (this->m_filter_info == NULL) return RTC_NG;
    if (this->m_filter_info->m_param == NULL) return RTC_NG;
    FilterParam *param = this->m_filter_info->m_param;

    LOGOUT(FILTER_LOG, (char*)"=====================================================\n");
    LOGOUT(FILTER_LOG, (char*)"==========       Read Write Solution       ==========\n");
    LOGOUT(FILTER_LOG, (char*)"=====================================================\n");
    int m;
    int err_state = RTC_OK;
    char filePath[FILELEN_MAX];
    sprintf(filePath, "%s/%s%06d.%s", param->in_dir, data_set->params->solution_params.file_prefix, step, data_set->params->solution_params.file_suffix);
    DEBUG(3," Trying to open filepath %s\n", filePath);
    err_state |= plot3d_dataSetS_openStepFiles(data_set);
...
int FilterIoPlot3d::plot3d_dataSetS_readCopySolutionBlock(Plot3Ddataset* data_set, float* target, int step, int block, int skip) {
    if (this->m_filter_info == NULL) return RTC_NG;
    if (this->m_filter_info->m_param == NULL) return RTC_NG;
    ...
    varTypeDataArray_getComponentRanges(&(sGrid->QValues), info->m_value_min, info->m_value_max, step * 5);
    plot3d_solutionBlock_deAllocateMemory(sGrid);
    plot3d_dataSetS_closeStepFile(data_set, step);
    return err_state;
};

int FilterIoPlot3d::plot3d_dataSetS_getCopySolutionBlock(Plot3Ddataset* data_set, float* target, int block, int skip) {
    DEBUG(3,"----------------------------------------- \n");
    int err_state = RTC_OK;
    Plot3DSolutionBlock* sGrid;
    LOGOUT(FILTER_LOG, (char*)"#### Copy cached solution block  %i   into m_values[]\n", block);
    sGrid = &(data_set->solution_blocks[block]);
    DataArray_copyAsFloat(&(sGrid->QValues), target, 0, skip);
    return err_state;
};
