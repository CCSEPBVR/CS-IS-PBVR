 ...
 *
 */
void FilterLog::get_localtime(char* time_buff) {
#ifdef MSDOS
    struct _timeb tb;
#else
    struct timeb tb;
 ...
 *
 */
void FilterLog::filter_log(int logmode, char* format, ...) {
    FILE *fp_log = NULL;
    char logfile[LOGNAME_MAX];

    va_list p;
    char *env_p = NULL;
    char buff[LOGNAME_MAX] = {0x00};
...
void FilterLog::f_log(int line, const char* fname, const char* func) {

    this->log_line = line;
    if (strcmp(this->log_file, fname)) {
#ifdef DEBUG_LEVEL
    printf("\n  **File|: %s :|**", fname);
#endif
    sprintf(this->log_file, "%s", fname);
    }
    if (strcmp(log_func, func)) {
#ifdef DEBUG_LEVEL
    printf("\nFunction|: %s :|**\n", func);
#endif
    sprintf(this->log_func, "%s", func);
    }
#ifdef DEBUG_LEVEL
    printf("   %5d|:", line);
#endif
